...
Notice that this code doesn't specify which model provider to use - it simply describes _what_ we want to do (generate a dad joke), not _how_ or _where_ to do it.
...
// - Provides an LanguageModel which uses OpenAi's Chat Completions API
...
const concurrentDadJokes = Effect.all(
  [generateDadJoke, generateDadJoke, generateDadJoke],
  {
    concurrency: 2,
  },
...
// Use `LanguageModel.streamText` to stream parts of a response from a large
