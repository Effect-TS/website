...
<Info>
  For simplicity, let's assume that we won't encounter any errors during
  the value construction (meaning `E = never`).
</Info>
...
<Idea>
  ...
  for the "test" implementation. For example, for a `Database` service, the
