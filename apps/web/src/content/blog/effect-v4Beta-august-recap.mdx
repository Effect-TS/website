---
pagefind: false
title: "Effect v4 Beta/RC: August 2026 Updates"
excerpt: "Release Candidate announcement, zero external dependencies in core, a native PostgreSQL client, pull-based Sockets, native Schema Arbitraries, and more."
date: 2026-08-31
authors:
  - davide_scognamiglio
  - mirela_prifti
...
On August 12th, the team tagged the first [Release Candidate](https://effect.website/blog/releases/effect/40-rc), signaling that the API surface is presumed final and the focus has fully shifted to correctness, validation, and documentation.
The following weeks brought a sustained hardening effort, plus a standout milestone in the final days: **the Effect v4 core package now has zero external dependencies**.

## What landed in August

### Effect v4 Release Candidate

The biggest news of the month: **Effect v4 is now in Release Candidate**. After months of beta releases, real-world usage, and a deep audit of the entire codebase, the team is confident the sweeping changes are behind them. No broad breaking changes are planned for the RC cycle, any change that does land will be narrow in scope, clearly communicated, and accompanied by a migration path.

The RC is an invitation to migrate real applications and validate against production workloads. If you find something that should stop the stable release, the team wants to know now.
...
Schema's Arbitrary generation and shrinking was reworked with a native implementation, replacing the previous approach for more predictable and efficient property-based test data generation, with no external dependency.
