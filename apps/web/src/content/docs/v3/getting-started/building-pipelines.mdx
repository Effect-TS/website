...
Both [Option](/docs/v3/api/effect/Option) and [Either](/docs/v3/api/effect/Either) are commonly used for handling optional or missing values or simple error cases. These types integrate well with `Effect.andThen`. When used with `Effect.andThen`, the operations are categorized as scenarios 5 and 6 (as discussed earlier) because both `Option` and `Either` are treated as effects in this context.
...
See [Collecting](/docs/v3/getting-started/control-flow/#all) for more information on how to use `Effect.all`.
