...
If you've used `AiModel` before (via `OpenAiLanguageModel.model()` or similar), you'll find `AiPlan` familiar. In fact, an `AiModel` *is* in fact an `AiPlan` with just a single step.
...
```ts twoslash {24-28} collapse={7-15} 
import { AiPlan } from "@effect/ai"
import type { AiLanguageModel, AiResponse } from "@effect/ai"
