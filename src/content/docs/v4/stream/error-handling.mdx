...
You can also distinguish elements from the two streams based on success or failure by tagging each side with the [Result](/docs/data-types/result/) data type before merging them, mapping `s1`'s elements into `Result.succeed` and `s2`'s elements into `Result.fail`:
...
const res = Stream.catch(stream, (error) => {
  const refined = error instanceof SyntaxError ? Option.some(error) : Option.none()
...
const stream = Stream.fromEffect(Effect.never).pipe(
  Stream.timeoutOrElse({ duration: "2 seconds", orElse: () => Stream.fail("timeout") }),
)
...
const stream = Stream.fromEffect(Effect.never).pipe(
  Stream.timeoutOrElse({ duration: "2 seconds", orElse: () => Stream.make(1, 2, 3) }),
)
