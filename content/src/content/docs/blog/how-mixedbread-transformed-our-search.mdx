...
As I mentioned before, our docs site is built using the [Starlight Documentation Plugin for Astro](https://starlight.astro.build/guides/site-search/). To provide a seamless experience, we built a small, custom [Starlight Plugin](https://starlight.astro.build/reference/plugins/) which replaces the default Pagefind search. This plugin [overrides the `Search` component](https://starlight.astro.build/guides/overriding-components/) that comes out of the box with Starlight with a custom `MixedbreadSearch` Astro component. The `MixedbreadSearch` Astro component sends the user's search query to an [Astro API route](https://docs.astro.build/en/guides/endpoints/#server-endpoints-api-routes) which executes the search against our Mixedbread vector store. Results are returned to the client and rendered by the `MixedbreadSearch` Astro component.
