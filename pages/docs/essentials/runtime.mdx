...
To simplify, we can envision the Runtime System as a black box that takes both the effect ([`Effect<R, E, A>`](../concepts/effect-data-type)) and its associated environment (`Context<R>`). It runs the effect and returns the result as an [`Exit<E, A>`](exit) value.
