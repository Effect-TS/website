...
const AppConfigLive = Layer.merge(ConfigLive, LoggerLive).pipe(Layer.provide(ConfigLive))
...
await Effect.runPromise(
  Effect.provide(Database.pipe(Effect.andThen((db) => db.query("SELECT * FROM users"))), MainLive),
) // => { result: "Results from mysql://username:password@hostname:port/database_name" }
```
...
When constructing layers, it is important to handle potential errors. The Effect library provides tools like `Layer.catch` and `Layer.orElse` to manage errors and define fallback layers in case of failure.
...
The `Layer.orElse` function provides a simpler way to fall back to an alternative layer if the initial layer fails. Unlike `Layer.catch`, it does not receive the error as input. Use this when you only need to provide a default layer without reacting to specific errors.
...
const database = postgresDatabaseLayer.pipe(Layer.catch(() => inMemoryDatabaseLayer))
...
`Context.Service` lets you define a service's tag and its implementation together, as a single class, instead of declaring the tag and building the layer separately.
...
class Cache extends Context.Service<Cache>()("app/Cache", {
  ...
  static readonly layer = Layer.effect(this, this.make).pipe(Layer.provide(NodeFileSystem.layer))
}
...
Unlike v3's `Effect.Service`, `Context.Service` does not generate any layers for you — you declare them yourself as static class fields, built from `this.make` with `Layer.effect`. The field names below are just a convention, not part of the API.
...
`Context.Service`'s `make` field accepts any `Effect`, so the named constructor options v3's `Effect.Service` used to offer are now just different ways to build that one `Effect` value:
