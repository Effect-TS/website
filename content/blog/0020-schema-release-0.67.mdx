...
Now, you can easily access `Type` and `Encoded` directly from a schema without the need for `Schema.Schema.Type` and `Schema.Schema.Type`.
...
Notice how the inferred type for `MyUnion` now refers only to the class types (`typeof Person`, `typeof Group`),
