...
If we fast forward to today we can easily see that things have changed a lot, the web platform offers incredible capabilities and most of the programs we are used to interact with are fully built on the web.
...
we won't see any "MyComponent Invoked" messages in the browser console, that's because a component was created but it wasn't rendered as it isn't part of the returned UI description. 

This proves that simply creating a component doesn't perform any side-effect, it is a pure operation, even if the component itself contains side-effects.
...
produces a few "MyComponent Invoked" messages, which means side-effects are being performed, how many depends on your setup, if you are running with Strict Mode or not and if you are in development or not (detail that can be ignored).
...
This seems a little useless but if your component does something interesting like modelling a button this can be quite powerful as you can reuse the button code in multiple places without rewriting it.

Further a component may crash and throw an error, such error can be recovered from a parent component and actions such as rendering an alternative UI can be taken.
...
Arguably the API to catch errors in components isn't very nice, but also it isn't too common to throw from components, the only real case where one would throw in a component is throwing a promise that is awaited from the nearest suspense boundary, making it possible to have async work in components.
...
This API is fairly low level and there are libraries that leverages it internally in order to provide features such as smooth data fetching (mandatory shutout to [React Query](https://tanstack.com/query/latest/docs/framework/react/guides/suspense)) and data streaming from SSR with server components (fairly new buzz).

Additionally, by being a **description**, a React component can access contextual data provided by parent components, lets have a look:
...
You might ask, why so much attention to React? How is it linked to Effect? similarly to why React was and is important for UI development Effect is equally important for general purpose code. Over the past two decades JS & TS evolved a lot, and thanks to the ideas brought forward by Node.js we now develop full stack applications on top of what people initially though was a toy language. 
...
An Effect can represent **any** piece of code, let it be a console.log statement, a fetch call, a database query or a computation, in fact Effect is capable of executing both async and sync code in a unified model, escaping the issue of <a href="https://journal.stuffwithstuff.com/2015/02/01/what-color-is-your-function/" target="_blank">function coloring</a> (having different types for async or sync).

Just like React components are able to access context provided by a parent component also Effects can access context provided from a parent Effect, let's see how:
...
Differently from React we don't have to provide a default implementation for our context, Effect will keep track of the requirements in its third type parameter and it will prohibit execution of an Effect that doesn't have all requirements fullfilled, you can check the type of `printFromContext` and will see `Effect<void, never, ContextualData>` which means that this Effect will return `void` upon success and that it needs `ContextualData` in order to become executable.

## Conclusion

We can see that Effect and React use basically the same model, both are about making composable descriptions that are later executed by a runtime, only the domain is different while React focuses on UI Effect focuses on general purpose code.
...
If you're curious your next read should be [the docs](https://effect.website/docs/introduction) and your next watch should be the [Effect Beginner Workshop](https://www.youtube.com/watch?v=Lz2J1NBnHK4).
