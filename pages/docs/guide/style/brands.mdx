...
In this tutorial, we will explore the concept of **branded types** in TypeScript and learn how to create and work with them using the `@effect/data/Brand` module. Branded types are TypeScript types with an added type tag that helps prevent accidental usage of a value in the wrong context. They allow us to create distinct types based on an existing underlying type, enabling type safety and better code organization.

## Setup

Before we dive into creating and using branded types, make sure you have the `@effect/data/Brand` module installed in your TypeScript project. You can install it via npm:

```bash filename="Terminal"
npm install @effect/data
```

Once installed, you can import the necessary functions and types from the module:
...
The `@effect/data/Brand` module provides two main functions for creating branded types: `refined` and `nominal`. Let's understand each of them:
...
function createUser(name: string, email: string): UserId {
...
In some scenarios, you may need to combine multiple branded types together. The `@effect/data/Brand` module provides the `all` API to facilitate this:
