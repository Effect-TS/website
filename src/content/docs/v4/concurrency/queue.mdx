...
const program = Effect.gen(function* () {
  ...
  const chunk = yield* Queue.takeBetween(queue, 0, 2)
  console.log(chunk)
...
The `Queue.await` operation can be used to run an effect when the queue shuts down. It waits until the queue is closed and resumes immediately if the queue is already shut down.
