...
The `Effect.timeout` function employs a [Duration](/docs/v3/api/effect/Duration) parameter to establish a time limit on an operation. If the operation exceeds this limit, a `TimeoutException` is triggered, indicating a timeout has occurred.
...
If you want to handle timeouts more gracefully, consider using `Effect.timeoutOption`. This function treats timeouts as regular results, wrapping the outcome in an [Option](/docs/v3/api/effect/Option).
...
**Example** (Handling Success and Timeout with [Either](/docs/v3/api/effect/Either))
