---
title: Keep AI-generated code under control
...
<TutorialPainPoint title="LLMs produce plausible-looking slop">
  ...
  hiding failures, trusting unvalidated data, or describing Effect operations
...
the diagnostics expose three concrete mistakes. The same evidence then defines
one focused correction before the endpoint is verified again.
...
- `server.ts` is the application entry point. It is the file changed by the
  coding agent and then corrected during the tutorial.
- `orders.json` supplies predictable external data for the import endpoint. It
  stays unchanged.
- `package.json` configures ES modules and records the dependencies and commands
  added during setup.
- `.oxlintrc.json` configures the Effect-aware diagnostics. It is added once and
  then stays unchanged.

Every version of `server.ts` is shown as a complete file. The example uses an ES
module project. Node.js 22.18.0 or newer is required because the tutorial runs
`server.ts` directly with Node's built-in TypeScript support.
...
The server runs in one terminal pane with the following command. `--watch`
makes Node.js reload `server.ts` after each edit, so the same process can remain
...
rules from the command line. These commands install the runtime and compatible
Oxlint tooling:

```sh title="Effect and static diagnostics"
npm install effect@4.0.0-beta.104
npm install --save-dev @types/node@22 @effect/tsgo oxlint oxlint-tsgolint
...
- [Why Effect keeps AI-generated code under control](/docs/v4/get-started/writing-effect-with-ai)
