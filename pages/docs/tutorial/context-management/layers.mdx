...
const program = pipe(
  ...
  Effect.flatMap((steps) => Effect.forEachDiscard(steps, Effect.logInfo))
)
...
The `Effect.forEachDiscard` function is used to iterate over a collection of values (`steps`) and perform a side effect on each element (`Effect.logInfo`), discarding the result (`void`).
