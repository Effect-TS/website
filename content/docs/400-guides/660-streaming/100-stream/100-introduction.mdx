...
Think of a `Stream` as an extension of an `Effect`. While an `Effect<R, E, A>` represents a program that requires a context of type `R`, may encounter an error of type `E`, and always produces a single result of type `A`, a `Stream<R, E, A>` takes this further by allowing the emission of zero or more values of type `A`.
