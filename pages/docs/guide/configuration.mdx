...
```ts filename="withDefault.ts" /Config.withDefault(8080)/ file=<rootDir>/src/guide/configuration/withDefault.ts
...
```ts filename="App.ts" {5} file=<rootDir>/src/guide/configuration/App.ts
...
```ts filename="App.ts" {6} file=<rootDir>/src/guide/configuration/App-gen.ts
