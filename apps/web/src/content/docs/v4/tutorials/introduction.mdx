...
The diagram keeps every component in a stable position while highlighting the
components and relationships affected by one tutorial. Select a tutorial to
explore its focus. The dimmed components still exist in the scenario, but the
selected tutorial keeps them behind simple fixtures so they do not distract
from its lesson.
