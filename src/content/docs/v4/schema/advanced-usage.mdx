...
export const MyReadonlySet = <A, I, R>(
  ...
  item: Schema.Codec<A, I, R>,
) =>
  Schema.declareConstructor<ReadonlySet<A>, ReadonlySet<I>>()(
...
const arb = Schema.toArbitrary(FileFromSelf)
/*
throws:
...
In the above example, attempting to generate arbitrary values for the `FileFromSelf` schema fails because the interpreter lacks the necessary annotations. To resolve this, you need to provide annotations for generating arbitrary data:
...
const Person = Schema.Struct({
  name: Schema.String,
  age: Schema.NumberFromString, // Maps from "AGE" to "age"
...
const Product = Schema.Struct({
  quantity: Schema.optional(Schema.NullOr(Schema.NumberFromString)).pipe(
    Schema.decodeTo(Schema.optional(Schema.Number), {
      decode: SchemaGetter.transformOptional((o) => o.pipe(Option.filter(Predicate.isNotNull))),
...
//      ┌─── typeof Schema.NumberFromString
//      ▼
const from = Product.fields.quantity.schema
```
...
const Product = Schema.Struct({
  quantity: Schema.OptionFromOptional(Schema.NumberFromString),
})
...
const Product = Schema.Struct({
  quantity: Schema.OptionFromOptionalKey(Schema.NumberFromString),
})
...
const Product = Schema.Struct({
  quantity: Schema.OptionFromOptionalNullOr(Schema.NumberFromString),
})
...
const Extended = UnionOfStructs.mapMembers(Tuple.map(Schema.fieldsAssign(Struct.fields)))
...
const Renamed = Schema.Union([
  Schema.Struct({ a: Schema.String, b: Schema.Number }).pipe(Schema.encodeKeys({ a: "c" })),
  Schema.Struct({ a: Schema.String, d: Schema.Boolean }).pipe(Schema.encodeKeys({ a: "c" })),
])
