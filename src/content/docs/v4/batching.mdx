...
A `RequestResolver<A>` is capable of executing requests of type `A`. Unlike in v3, it no longer carries a separate environment type parameter — the resolver passed to `Effect.request` must have no requirements of its own (see [Resolvers with Context](#resolvers-with-context) below for how to resolve services before constructing one).
...
const GetUserByIdResolver = RequestResolver.make(
  (entries: ReadonlyArray<Request.Entry<GetUserById>>) =>
    Effect.tryPromise({
      ...
      Effect.catch((error) =>
        Effect.forEach(entries, (entry) => Request.completeEffect(entry, Effect.fail(error))),
      ),
    ),
)

// Assuming SendEmail can be batched, we create a batched resolver
const SendEmailResolver = RequestResolver.make((entries: ReadonlyArray<Request.Entry<SendEmail>>) =>
  Effect.tryPromise({
    try: () =>
    ...
    Effect.andThen(Effect.forEach(entries, (entry) => Request.completeEffect(entry, Effect.void))),
    Effect.catch((error) =>
      Effect.forEach(entries, (entry) => Request.completeEffect(entry, Effect.fail(error))),
    ),
  ),
...
// Define a request to get multiple Todo items which might
// fail with a GetTodosError
interface GetTodos extends Request.Request<Array<Todo>, GetTodosError, HttpService> {
  readonly _tag: "GetTodos"
}
...
class HttpService extends Context.Service<HttpService, { fetch: typeof fetch }>()("HttpService") {}
...
Unlike in v3, caching is no longer an ambient toggle applied at the call site. Instead, `RequestResolver.withCache` wraps a resolver in a bounded, in-memory cache keyed by request equality — the resolver itself decides whether it caches, and any query built on top of it (via `Effect.request`) inherits that behavior automatically.
...
const getUserById = (id: number) => Effect.request(GetUserById({ id }), cachedGetUserByIdResolver)
...
const fifoCachedResolver = Effect.runSync(
  RequestResolver.withCache(GetUserByIdResolver, { capacity: 256, strategy: "fifo" }),
)
```

Cache entries created this way don't expire by time. If you need time-to-live-based expiry, or want cache lookups exposed as a first-class `Cache` (with `get`/`refresh`/`invalidate`, see [Cache](/docs/caching/cache/)) instead of a plain `RequestResolver`, use [`RequestResolver.asCache`](https://effect-ts.github.io/effect/effect/RequestResolver.ts.html) instead — it accepts the same `capacity` option plus an optional `timeToLive`. For caching that survives process restarts, see `RequestResolver.persisted`.
