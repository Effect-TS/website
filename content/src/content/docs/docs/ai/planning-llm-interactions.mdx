...
Think of an `AiPlan` as the **blueprint** for executing a model, with logic for when to keep trying and when to switch strategies.
...
```ts twoslash collapse={2-15,18-21}
import { AiPlan } from "@effect/ai"
import type { AiResponse, Completions } from "@effect/ai"
