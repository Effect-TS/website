...
There's also a variant called `Stream.orElseEither` that uses the [Either](/docs/v3/api/effect/Either) data type to distinguish elements from the two streams based on success or failure:
