...
<Aside type="caution" title="Shallow Comparison">
  `Equal.equals` only checks the top-level structure. Use `Data` constructors
  for nested objects if you need deep comparisons.
</Aside>
...
You can yield an instance of `NotFound` directly in an [Effect.gen](/docs/v3/getting-started/using-generators/), without needing to use `Effect.fail`.
...
Effect provides a `TaggedError` API to add a `_tag` field automatically to your custom errors. This simplifies error handling with APIs like [Effect.catchTag](/docs/v3/error-management/expected-errors/#catchtag) or [Effect.catchTags](/docs/v3/error-management/expected-errors/#catchtags).
