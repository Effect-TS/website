...
The `Effect.addFinalizer` function is a high-level API that allows you to add finalizers to the scope of an effect. A finalizer is a piece of code that is guaranteed to run when the associated scope is closed. The behavior of the finalizer can vary based on the [Exit](/docs/v3/data-types/exit/) value, which represents how the scope was closed—whether successfully or with an error.
...
**Example** (Adding a Finalizer on [Interruption](/docs/v3/concurrency/basic-concurrency/#interruptions))
...
To begin, we define the domain model for the required [services](/docs/v3/requirements-management/services/):
...
To achieve this, we will utilize [layers](/docs/v3/requirements-management/layers/) to construct test
