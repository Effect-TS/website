...
Effect.runCallback(readTodos, (result) =>
  Exit.match(
    result,
    (cause) => console.error(cause),
    (todos) => console.log(`I'm done! ${todos}`)
  )
)
```

In this example, `readTodos` is an Effect that reads the contents of a file asynchronously using the Node.js `fs` module.
The `runCallback` function executes the Effect and provides a callback function to handle the result.
Inside the callback function, we use `Exit.match` to pattern match on the result and handle both success and failure cases.
If an error occurs while reading the file, we log the cause of the error, and if the reading is successful, we log a success message
along with the todos.
