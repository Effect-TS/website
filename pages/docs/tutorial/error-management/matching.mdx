...
```ts file=<rootDir>/src/tutorial/error-management/matching/match.ts

```

We can also choose to ignore the success and failure values if we're not interested in them:

```ts /never, void/ file=<rootDir>/src/tutorial/error-management/matching/ignore-constVoid.ts
...
```ts /never, void/ file=<rootDir>/src/tutorial/error-management/matching/ignore.ts
...
```ts file=<rootDir>/src/tutorial/error-management/matching/matchEffect.ts
...
```ts file=<rootDir>/src/tutorial/error-management/matching/matchCauseEffect.ts
