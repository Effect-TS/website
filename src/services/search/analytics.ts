...
export type SearchFailureReason = "http" | "invalid_response" | "network" | "timeout"
...
interface PostHogClient {
  readonly capture: (
    ...
    options?: { readonly send_instantly?: boolean; readonly transport?: "sendBeacon" },
  ) => void
}
...
function queryProperties(query: string): Readonly<Record<string, AnalyticsProperty>> {
  ...
  return {
    query_length_bucket:
      length <= 5 ? "1-5" : length <= 10 ? "6-10" : length <= 25 ? "11-25" : "26+",
    query_word_count_bucket: words <= 1 ? "1" : words <= 3 ? "2-3" : words <= 6 ? "4-6" : "7+",
  }
}
...
function resultCounts(results: ReadonlyArray<SearchResult>) {
  return {
    ...
    selectable_result_count: results.reduce((count, result) => count + result.chunks.length + 1, 0),
    documentation_result_count: results.filter((result) => result.kind === "documentation").length,
    api_reference_result_count: results.filter((result) => result.kind === "api-reference").length,
    blog_result_count: results.filter((result) => result.kind === "blog").length,
  } as const
}

export const SearchAnalytics = {
  dialogOpen(source: SearchOpenSource, version: string, hasExistingQuery: boolean): void {
    session = {
  ...
  requestComplete(query: string, durationMs: number, results: ReadonlyArray<SearchResult>): void {
  ...
  resultClick(properties: {
    ...
    capture(
      "search:result_click",
      {
        ...
        ...(properties.chunkRank === undefined ? {} : { chunk_rank: properties.chunkRank }),
