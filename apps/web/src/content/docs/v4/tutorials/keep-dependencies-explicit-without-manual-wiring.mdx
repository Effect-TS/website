...
- `fixture.ts` provides a ready-made HTTP server and deterministic
  Promise-based implementations of a payment provider and an order repository.
- `server.ts` is the application entry point. It contains the order logic and
  the infrastructure setup that we will improve throughout the tutorial.
...
The files run directly with Node.js, so they require Node.js 22.18.0 or newer
for built-in TypeScript support. The project also needs the Effect version used
by these docs:
...
After copying any current `server.ts` block, the server can run in one terminal
pane. The `--watch` option reloads it after each saved edit, so the same process
can remain running throughout the tutorial:
