...
const schema = Schema.NumberFromString.check(Schema.isBetween({ minimum: 1, maximum: 10 }))
...
const MyNumber = Schema.Number.check(Schema.isBetween({ minimum: 1, maximum: 10 }))
...
const safeMake = Schema.decodeResult(Schema.toType(schema))
...
```ts twoslash import.meta.vitest name="nested-structs-and-shallow-defaults-1"
import { Effect, Schema } from "effect"
