...
Plain JavaScript objects, arrays, tuples, `Map`s, and `Set`s get structural equality with `Equal.equals` by default — no special constructor is required. See [Equal](/docs/trait/equal/) for the full explanation.
