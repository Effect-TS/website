...
const host = Config.string("HOST").pipe(Config.orElse(() => Config.string("FALLBACK_HOST")))
...
The v4 configuration API is schema-based. Common migrations include:

| v3 usage                                      | v4 replacement                                                         |
| --------------------------------------------- | ---------------------------------------------------------------------- |
| `Config.integer`                              | `Config.int`                                                           |
| `Config.validate`                             | A check passed through `Config.schema`                                 |
| `Config.array` or `Config.repeat`             | `Config.schema(Config.Array(valueSchema), path)`                       |
| `Config.hashMap`                              | `Config.schema(Config.Record(keySchema, valueSchema), path)`, then map |
| `ConfigProvider.fromJson`                     | `ConfigProvider.fromUnknown`                                           |
| Installing a provider with an Effect operator | `config.parse(provider)` or `ConfigProvider.layer(provider)`           |

The v3 collection constructors (`chunk`, `hashSet`, and similar helpers) are no longer `Config` combinators. Decode an array or record with a schema, then map it to the desired collection type.
