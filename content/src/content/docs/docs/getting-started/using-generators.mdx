...
One way to handle errors within `Effect.gen` is encapsulate their outcomes into [Either](/docs/data-types/either/). `yield*` will only stop computation on failed effects, so if you encapsulate an effect into an either then you can get the value of the either and write control flow based on whether it's left (encapsulating a failure) or right (a success).
...
You can also encapsulte effects into the [Exit](/docs/data-types/exit/) type using [Effect.exit](https://effect-ts.github.io/effect/effect/Effect.ts.html#exit): the main difference being that Exit objects also handle [defects, like Effect.die](/docs/error-management/unexpected-errors/), and they wrap failures in [Causes](https://effect.website/docs/data-types/cause/).

In most cases, encapsulating effects as either is easier, because you don't have to deal with extracting the original error out of a Cause. However, if you want to handle defects in addition expected errors, using the exit type is the best approach.
