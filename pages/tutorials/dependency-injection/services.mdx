...
When we need to manage multiple services in our program, we can utilize the `all` function.
...
const context: Context.Context<Random | Logger> = pipe(
  Context.empty(),
  Context.add(Random, { next: () => Effect.succeed(Math.random()) }),
  Context.add(Logger, {
    log: Effect.logInfo,
  })
)
...
In the upcoming section, we will delve into the topic of [Layers](layers).
Layers are powerful constructs for creating services, enabling us to build complex contexts that can be seamlessly provided to Effects.
...
<Cards>
  <Card icon="📚 " title="Building Dependency Graph" href="layers" />
</Cards>
