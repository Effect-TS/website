...
<Aside type="note" title="Extending a Scope">
  The `Scope.provide` function allows you to extend the scope of an effect workflow that requires a
...
await Effect.runPromise(Effect.scoped(Effect.map(resource, (res) => res.contents))) // => "lorem ipsum"
```
...
class FailureCase extends Context.Service<FailureCase, FailureCaseLiterals>()("FailureCase") {}
