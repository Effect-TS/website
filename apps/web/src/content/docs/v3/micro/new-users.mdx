...
The `Micro.either` function transforms an `Micro<A, E, R>` into an effect that encapsulates both potential failure and success within an [Either](/docs/v3/api/effect/Either) data type:
...
If you want to handle the result of whichever task completes first, whether it succeeds or fails, you can use the `Micro.either` function. This function wraps the result in an [Either](/docs/v3/api/effect/Either) type, allowing you to see if the result was a success (`Right`) or a failure (`Left`):
