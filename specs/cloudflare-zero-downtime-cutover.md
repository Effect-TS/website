...
1. **Generated Open Graph cache keys are incomplete.** Most generated image URLs
   ...
   metadata.
2. **Playground filesystem acknowledgements can become stale.** Completed write
...
3. **Expired Mixedbread stores can be treated as healthy.** The provider
...
- [x] Legacy `/docs/api/v4` paths redirect permanently to their `/docs/v4/api`
      equivalents, preserving the remaining path and query string.
