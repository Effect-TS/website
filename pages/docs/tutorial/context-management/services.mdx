...
Remember that in the `Effect<R, E, A>` type, the parameter `R` represents the contextual data required by the effect to be executed. This contextual data is stored in a collection called `Context`.
...
- How to provide the required context to the Effect
...
We use the `Effect.provideService(tag, implementation){:ts}` function to associate the `Random` service with its implementation, which is an object with a `next` method that generates a random number.
