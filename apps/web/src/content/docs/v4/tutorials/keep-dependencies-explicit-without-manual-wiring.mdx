...
<EffectStrength title="Dependencies stay explicit without manual wiring">
...
## Move dependency selection out of business logic
...
another side effect. `Effect.Effect` is the
...
`Effect.tryPromise` adapts the two
...
`Effect.flatMap` uses the successful
...
`Effect.provideService` supplies
...
`Effect.map` transforms the successful
...
`Layer.succeed` creates a Layer for one
...
`Layer.Layer` is the type of a Layer
...
`Effect.provide` constructs the supplied
