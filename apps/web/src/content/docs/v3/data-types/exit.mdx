...
- `Exit.Failure`: Contains a failure [Cause](/docs/v3/data-types/cause/) of type `E`.
...
The failure is described using a [Cause](/docs/v3/data-types/cause/) object, which can encapsulate expected errors, defects, interruptions, or even composite errors.
...
Conceptually, `Exit<A, E>` can be thought of as `Either<A, Cause<E>>`. However, the [Cause](/docs/v3/data-types/cause/) type represents more than just expected errors of type `E`. It includes:
