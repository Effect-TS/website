...
The `Effect.loop` function allows you to repeatedly change the state based on an `step` function until a condition given by the `cont` function is evaluated to `true`:
...
<Info>
  By default, `forEach` executes all effects sequentially. To manage concurrency
  and explore other options, you can refer to the [Concurrency
  Options](./concurrency/concurrency-options) section.
</Info>
