...
The `@effect/sql-kysely` package provides `@effect/sql` integration with the `@effect/kysely`
query builder apis.
...
It the `Iterable` is not "NonEmpty", then the Effect can fail with a `Cause.NoSuchElementException`.
...
If the `onlyEffect` option for `Effect.tap` is set to `true`, then it will only allow effects to be provided.

This can be useful when you want to add strictness to your program.

## Refinement support for Predicate.tuple/struct

Refinements will now narrow the resulting type when used with `Predicate.tuple` and `Predicate.struct`.
