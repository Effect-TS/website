...
An `Exit<A, E>` is conceptually an `Either<Cause<E>, A>`. However, it's important to note that [`Cause`](cause) encompasses more states than just the expected error type `E`. It also includes other states such as interruption and defects (unexpected errors), as well as the possibility of combining multiple `Cause` values together.
