...
equivalence({ name: "John", age: 23, tags: ["admin"] }, { name: "John", age: 23, tags: ["admin"] }) // => true

equivalence({ name: "John", age: 23, tags: ["admin"] }, { name: "John", age: 24, tags: ["admin"] }) // => false
```
...
`Schema.Any`, `Schema.Unknown`, `Schema.ObjectKeyword`, and an empty `Schema.Struct({})` use `Equal.equals`. In v4, `Equal.equals` performs deep structural comparison for objects and arrays rather than defaulting to reference equality.
