...
const stream = groupByKeyResult.pipe(
  Stream.flatMap(
    ([key, stream]) =>
      Stream.fromEffect(
        Stream.runCollect(stream).pipe(Effect.map((values) => [key, values.length] as const)),
      ),
...
const groupByKeyResult = Stream.fromIterable([
  ...
  Stream.groupBy((name) => Effect.succeed([name.substring(0, 1), name] as const)),
)
...
Effect.runPromise(Stream.runCollect(stream)).then((chunks) => console.log(chunks))
/*
Output:
[
  {
...
const stream2 = Stream.fromIterable([1, 2, 3]).pipe(Stream.switchMap(createStreamWithLogging))
...
const s2 = Stream.forever(Stream.succeed(0)).pipe(Stream.schedule(Schedule.spaced("200 millis")))
