...
`Cause<E>` no longer stores a recursive tree, so there is no separate representation for causes that occurred one after another versus causes that occurred concurrently. Both cases are represented the same way: a flat `reasons` array. Use `Cause.combine` to merge two causes into one.
...
/*
Output:
{ _id: 'Cause', failures: [ { _tag: 'Fail', error: 'Oh no!' } ] }
*/
```
...
for (const reason of cause.reasons) {
  if (Cause.isFailReason(reason)) {
    console.log(reason.error.message) // Output: my message
  }
}
```
...
There is no longer a single `Cause.match` combinator, because `Cause<E>` is a flat array of `reasons` rather than a tree with `Sequential` and `Parallel` nodes to fold over. Instead, iterate `cause.reasons` and switch on each reason's `_tag` to respond to specific error scenarios with custom behavior.
...
const cause = Cause.combine(Cause.fail(new Error("my fail message")), Cause.die("my die message"))
...
console.log(Cause.pretty(Cause.combine(Cause.fail("fail1"), Cause.fail("fail2"))))
/*
Output:
Error: fail1
    ...stack trace...
...
`Cause.failures` and `Cause.defects` were removed along with the recursive tree; filter `cause.reasons` directly with `Cause.isFailReason` and `Cause.isDieReason` to inspect only the errors or unexpected defects that occurred.
...
const program = Effect.gen(function* () {
  ...
  if (Exit.isFailure(exit)) {
    console.log(exit.cause.reasons.filter(Cause.isFailReason).map((reason) => reason.error))
    console.log(exit.cause.reasons.filter(Cause.isDieReason).map((reason) => reason.defect))
  }
})
