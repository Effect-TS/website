...
It's important to understand that `catchSomeDefect` can only handle defects, not [expected errors](expected-errors) (such as those caused by `Effect.fail`) or [interruptions](../concurrency/interruption-model) (TODO: broken link) in execution (such as when using `Effect.interrupt`).
