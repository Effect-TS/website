...
| `decodeUnknownOption`  | Decodes a value and returns an [Option](/docs/v3/data-types/option/) type.          |
| `decodeUnknownEither`  | Decodes a value and returns an [Either](/docs/v3/data-types/either/) type.          |
...
The `Schema.decodeUnknownEither` function allows you to parse a value and receive the result as an [Either](/docs/v3/data-types/either/), representing success (`Right`) or failure (`Left`). This approach lets you handle parsing errors more gracefully without throwing exceptions.
...
| `encodeOption`  | Encodes data and returns an [Option](/docs/v3/data-types/option/) type.                                 |
| `encodeEither`  | Encodes data and returns an [Either](/docs/v3/data-types/either/) type representing success or failure. |
...
Here is an example of a transformation that never fails during decoding. It returns an [Either](/docs/v3/data-types/either/) containing either the decoded value or the original input. For encoding, it is reasonable to not support it and use `Forbidden` as the result.
...
- **Decoding**: The `SafeDecode` function ensures that decoding never fails. It wraps the decoded value in an [Either](/docs/v3/data-types/either/), where a successful decoding results in a `Right` and a failed decoding results in a `Left` containing the original input.
...
The `Schema.decodeUnknownEither` and `Schema.encodeEither` functions returns a [Either](/docs/v3/data-types/either/):
