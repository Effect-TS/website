...
Unlike in v3, `Option` is no longer a structural subtype of `Effect`. It implements the `Yieldable` trait instead, so `yield*` still works inside `Effect.gen`, but passing an `Option` directly to an `Effect` combinator (such as `Effect.all`) requires an explicit conversion with `Effect.fromOption`.
...
You can compare `Option` values using the `Option.makeEquivalence` function. This function allows you to specify how to compare the contents of `Option` types by providing an [Equivalence](/docs/behaviour/equivalence/) for the type of value they may contain.
...
const sorted = Array.sortWith(items, (item) => item.data, Order.flip(Option.makeOrder(Order.Date)))
