...
Like `Fiber.await`, the `Fiber.interrupt` function returns an [Exit](/docs/v3/data-types/exit/) value that provides detailed information about how the fiber ended.
...
<Aside type="tip" title="Interrupting via Effect.interrupt">
  ...
  documentation](/docs/v3/concurrency/basic-concurrency/#interruptions).
</Aside>
...
Sometimes we want to create a fiber that is tied to a local [scope](/docs/v3/resource-management/scope/), meaning its lifetime is not dependent on its parent fiber but is bound to the local scope in which it was forked. This can be done using the `Effect.forkScoped` operator.
