...
const canFail: Effect.Effect<never, HttpError, never> = Effect.fail(
  new HttpError()
)
```
...
const flakyFoo = pipe(
  Random.next(),
  Effect.flatMap((n) =>
    n > 0.5 ? Effect.succeed("yay!") : Effect.fail(FooError())
  )
)
...
const flakyBar = pipe(
  Random.next(),
  Effect.flatMap((n) =>
    n > 0.5 ? Effect.succeed("yay!") : Effect.fail(BarError())
  )
)
...
const effectPromise: Effect.Effect<never, never, Error> = Effect.promise(
  () => promise
)
