...
All effects in Effect are executed by fibers. If you didn't create the fiber yourself, it was created by an operation you're using (if it's concurrent or parallel) or by the Effect runtime system.

Even if you write "single-threaded" code with no parallel or concurrent operations, there will always be at least one fiber: the "main" fiber that executes your effect.

Similar to operating system-level threads, Effect fibers have a well-defined lifecycle based on the effect they are executing.

Every fiber exits with either a failure or success, depending on whether the effect it is executing fails or succeeds.

Like operating system threads, Effect fibers have unique identities, local state, and a status (such as done, running, or suspended).
...
For example, the standard `Effect.zip` function combines two effects sequentially. However, there is also a concurrent version, `Effect.zip({_, _, { concurrent: true })`, which combines two effects in parallel.
...
In this example, we use the concurrent version of `Effect.zip` to run two tasks in parallel. The total duration will be approximately equal to the duration of the longest task, which is 2 seconds:
