...
Notice that the `HttpServer.serve` expression at the bottom has been converted
to use `pipe` instead of nesting function calls.
...
And in another shell we can test the endpoint. I'm using
[HTTPie](https://httpie.io/cli) throughout this tutorial, because the output is
nicer than `curl`.
...
Date: Fri, 07 Feb 2025 11:36:19 GMT
...
const users = Effect.flatMap(
  Effect.sync(() => ({ users: db.prepare("SELECT * FROM users").all() })),
