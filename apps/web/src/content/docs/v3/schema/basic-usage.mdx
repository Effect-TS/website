...
If you want to set a custom error message for the entire union of literals, you can use the `override: true` option (see [Custom Error Messages](/docs/v3/schema/error-messages/#custom-error-messages) for more details) to specify a unified message.
...
The `Schema.TemplateLiteral` constructor, while useful as a simple validator, only verifies that an input conforms to a specific string pattern by converting template literal definitions into regular expressions. Similarly, [`Schema.pattern`](/docs/v3/schema/filters/#string-filters) employs regular expressions directly for the same purpose. Post-validation, both methods require additional manual parsing to convert the validated string into a usable data format.
...
Here's how you can [transform](/docs/v3/schema/transformations/#transform) the `Shape` schema into another schema that represents a discriminated union:
...
you can set [`onExcessProperty`](/docs/v3/schema/getting-started/#managing-excess-properties) to `"error"`.
...
A common approach is to use [Schema.transform](/docs/v3/schema/transformations/#transform) to adjust keys during decoding.
...
By using [Schema.transform](/docs/v3/schema/transformations/#transform) and [Schema.compose](/docs/v3/schema/transformations/#composition),
...
The `Schema.instanceOf` constructor is just a lightweight wrapper of the [Schema.declare](/docs/v3/schema/advanced-usage/#declaring-new-data-types) API, which is the primitive in `effect/Schema` for declaring new custom data types.
...
In such cases, you cannot use `Schema.instanceOf`, and you must rely on [Schema.declare](/docs/v3/schema/advanced-usage/#declaring-new-data-types) like this:
...
To validate the fields of a class instance, you can use a [filter](/docs/v3/schema/filters/). This approach combines instance validation with additional checks on the instance's fields.
