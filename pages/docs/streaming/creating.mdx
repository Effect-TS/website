...
```ts file=<rootDir>/src/streaming/creating/make.ts
...
```ts file=<rootDir>/src/streaming/creating/empty.ts
...
```ts file=<rootDir>/src/streaming/creating/unit.ts
...
```ts file=<rootDir>/src/streaming/creating/range.ts
...
```ts file=<rootDir>/src/streaming/creating/iterate.ts
...
```ts file=<rootDir>/src/streaming/creating/scoped.ts
...
```ts file=<rootDir>/src/streaming/creating/fail-succeed.ts
...
```ts file=<rootDir>/src/streaming/creating/fromChunk.ts

```

Moreover, you can create a stream from multiple `Chunk`s as well:

```ts file=<rootDir>/src/streaming/creating/fromChunks.ts
...
```ts file=<rootDir>/src/streaming/creating/fromEffect.ts
...
Suppose you have an asynchronous function that relies on callbacks. If you want to register callbacks on that function and receive a stream of the results emitted by those callbacks, you can use `Stream.async`. This function can adapt functions that invoke their callbacks multiple times and emit the results as a stream:

```ts file=<rootDir>/src/streaming/creating/async.ts

```

The `register` function's error type is optional. By setting the error to `None`, you can use it to signal the end of the stream when needed. This allows you to handle asynchronous callback-based operations and collect their results in a stream.
...
```ts file=<rootDir>/src/streaming/creating/fromIterable.ts
...
```ts file=<rootDir>/src/streaming/creating/fromIterableEffect.ts
...
```ts file=<rootDir>/src/streaming/creating/repeatValue.ts
...
```ts file=<rootDir>/src/streaming/creating/repeat.ts
...
```ts file=<rootDir>/src/streaming/creating/repeatEffect.ts
...
```ts file=<rootDir>/src/streaming/creating/repeatEffectOption.ts
...
```ts file=<rootDir>/src/streaming/creating/tick.ts
...
```ts file=<rootDir>/src/streaming/creating/unfold.ts
...
```ts file=<rootDir>/src/streaming/creating/unfoldEffect.ts
...
```ts file=<rootDir>/src/streaming/creating/paginate.ts
...
```ts filename="domain.ts" file=<rootDir>/src/streaming/creating/unfolding-vs-pagination/domain.ts
...
```ts filename="firstAttempt.ts" file=<rootDir>/src/streaming/creating/unfolding-vs-pagination/firstAttempt.ts
...
```ts filename="secondAttempt.ts" file=<rootDir>/src/streaming/creating/unfolding-vs-pagination/secondAttempt.ts
...
```ts filename="finalAttempt.ts" file=<rootDir>/src/streaming/creating/unfolding-vs-pagination/finalAttempt.ts

```

## From Queue and Hub

In Effect, there are two essential asynchronous messaging data types: [`Queue`](../concurrency/queue) and [`Hub`](../concurrency/hub). You can easily transform these data types into `Stream`s by utilizing `Stream.fromQueue` and `Stream.fromHub`, respectively.
...
```ts file=<rootDir>/src/streaming/creating/fromSchedule.ts
