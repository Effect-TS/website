...
>()("EmbedManager", {
  make: Effect.fnUntraced(function* (
    ...
    const decodeEvent = Schema.decodeUnknownEffect(
      Schema.fromJsonString(YouTubeEvent),
    )
    const encodeCommand = Schema.encodeUnknownEffect(
      Schema.fromJsonString(EmbedCommand),
    )
    ...
    const ref = yield* RcRef.make({
      acquire: Effect.gen(function* () {
        ...
        const attemptHandshake = Effect.repeat(
          Effect.sync(sendListeningRequest),
          {
            times: 5,
    ...
    const connect = Effect.fn("EmbedManager.connect")(function* (
      element: HTMLElement,
    ) {
      const iframe = yield* RcRef.get(ref)
...
const applyPlayerInfoPatchToState = (
  state: EmbedState,
  ...
  switch (playerState) {
    ...
    case "ENDED":
      return PlaybackState.Ended({
        currentTimeSeconds: patch.duration ?? currentTime,
      })
    case "PAUSED":
