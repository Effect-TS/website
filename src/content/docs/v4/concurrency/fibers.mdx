...
Unlike `Fiber.await`, which returns an [Exit](/docs/data-types/exit/) value describing how a fiber completed, `Fiber.interrupt` returns `Effect<void>`. It still waits until the fiber has fully terminated (running all its finalizers along the way) before resuming, but it does not hand back the fiber's `Exit`.
...
const program = Effect.gen(function* () {
  ...
  const message = yield* Effect.catchCause(Fiber.join(fiber1), () => Fiber.join(fiber2))
  console.log(message)
  message // => "Hurray!"
})
