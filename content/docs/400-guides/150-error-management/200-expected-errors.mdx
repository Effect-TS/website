...
<Info>
  We use a class to represent the `HttpError` type above simply to gain access to both the error type and a free constructor. However, you can use whatever
...
<Idea>
  Adding a discriminant field, such as `_tag`, can be beneficial for distinguishing between different types of errors during error handling. It also
  prevents TypeScript from unifying types, ensuring that each error is treated uniquely based on its discriminant value.
</Idea>
...
<Warning>
  It is important to ensure that the error type used with `catchTag` has a `readonly _tag` discriminant field. This field is required for the matching and
  handling of specific error tags.
</Warning>
...
<Warning>
  It is important to ensure that all the error types used with `Effect.catchTags` have a `readonly _tag` discriminant field. This field is required for the
