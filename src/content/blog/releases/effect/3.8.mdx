...
Effect.gen(function* () {
  ...
  const fiber = yield* Effect.log("open sesame").pipe(latch.whenOpen, Effect.fork)
...
Effect.gen(function* () {
  ...
  yield* client.execute(HttpClientRequest.get("https://jsonplaceholder.typicode.com/todos/1"))
})
```
