...
the ability to specify the dependencies at the same time.
...
`Effect.provide` & `Layer.provide` now accept multiple layers to be provided.
...
Transform each element of a tuple using a function.
...
Adds elements to the end of an array until it reaches the desired length.
