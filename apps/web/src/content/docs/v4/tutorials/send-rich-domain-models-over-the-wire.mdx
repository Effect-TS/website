...
<TutorialPainPoint title="Rich domain models do not fit JSON">
  Useful domain models often contain classes, dates, and sets that JSON cannot
  represent directly. Sending them to a client usually introduces separate
  response types and conversion code that can drift away from the domain model.
...
| ------------------ | --------------- | ---------------------------------------------------------------- |
| Owned client       | API client      | Share the Schema and use its derived JSON representation         |
| External client    | API client      | Consume a fixed JSON contract                                    |
| HTTP routes        | HTTP boundary   | Encode the response for its intended client                      |
| Order domain model | Shared contract | Define rich values and their derived or explicit representations |
...
// This fixture stands in for the order-processing work used in other tutorials.
...
[`Schema.String`](/docs/v4/api/effect/Schema#String-variable) describes the
remaining JavaScript string. [`Schema.Class`](/docs/v4/api/effect/Schema#Class-variable)
...
[`Schema.Struct`](/docs/v4/api/effect/Schema#Struct-function) combines the field
schemas into one object Schema. [`Schema.Date`](/docs/v4/api/effect/Schema#Date-variable)
describes a valid JavaScript `Date` instance.
[`Schema.ReadonlySet`](/docs/v4/api/effect/Schema#ReadonlySet-function) describes
a set whose elements satisfy its inner Schema. [`Schema.Literal`](/docs/v4/api/effect/Schema#Literal-function)
...
[`Result.isFailure`](/docs/v4/api/effect/Result#isFailure) identifies the
...
[`Schema.DateFromMillis`](/docs/v4/api/effect/Schema#DateFromMillis-variable)
decodes epoch milliseconds into a JavaScript `Date` and encodes the `Date` back
into epoch milliseconds. [`Schema.Array`](/docs/v4/api/effect/Schema#Array-variable)
describes the array required for the external `labels` field.

[`Schema.decodeTo`](/docs/v4/api/effect/Schema#decodeTo-function) connects the
...
`ExternalAcceptedOrderFields.pipe(...)` passes the Schema into the next
function. Reading the code from top to bottom, `Schema.decodeTo(...)` receives
the external fields and returns the connected Schema. `pipe` only builds that
description; it does not encode or decode an order.
...
[`Schema.toType`](/docs/v4/api/effect/Schema#toType-function) exposes only the
