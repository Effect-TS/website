        ...
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-3 sm:gap-x-6">
          <div ref={categoryDropdownRef} className="relative">
            ...
            >
              ...
              <ChevronDown
                aria-hidden="true"
                className={`h-3.5 w-3.5 self-center text-muted-foreground transition-transform group-hover:text-foreground ${
          ...
          >
            ...
            <ArrowUpDown
              aria-hidden="true"
              className="h-3.5 w-3.5 self-center text-muted-foreground group-hover:text-foreground"
            />
          </button>
          <a
            href="/rss.xml"
            aria-label="RSS feed"
            className="group inline-flex items-baseline gap-1.5 font-mono text-sm text-muted-foreground transition-colors hover:text-foreground"
          >
            <span>RSS</span>
            <Rss
              aria-hidden="true"
      ...
      {paginatedPosts.length > 0 ? (
              ...
              >
                ...
                {pageItems.map((item, index) =>
                  item === "ellipsis" ? (
                    <span
                      key={`ellipsis-${index}`}
                      className="px-1.5 font-mono text-xs text-subtle-foreground"
                    >
                      ⋯
                    </span>
                  ) : (
                    ...
                    >
                      ...
                      <span
                        className={`pointer-events-none absolute right-2 -bottom-0.5 left-2 h-px origin-left bg-foreground transition-transform duration-300 ease-out ${
                          item === safePage
                            ? "scale-x-100"
        ...
        <div className="flex flex-col items-center justify-center py-24">
            ...
            {["release", "effect", "typescript"].map((suggestedId) => {
              ...
              return (
                <button
                  ...
                  className="inline-flex items-center rounded-md border border-border px-3 py-1.5 font-mono text-[10px] tracking-[0.12em] text-muted-foreground uppercase transition-colors hover:border-border-strong hover:text-foreground"
                >
                  {suggested.name}
                </button>
              )
          ...
          <button
            ...
            className="mt-6 rounded-md border border-border px-4 py-2 font-mono text-xs tracking-wider text-muted-foreground uppercase transition-colors hover:border-border-strong hover:text-foreground"
          >
            Clear all filters
          </button>
        </div>
