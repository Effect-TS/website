...
```ts twoslash import.meta.vitest name="creating-and-yielding-a-custom-error-1"
...
export const program = Effect.gen(function* () {
  ...
  return yield* new MyError({ message: "Oh no!" })
})
...
await Effect.runPromiseExit(program) // => Exit.fail(new MyError({ message: "Oh no!" }))
```
...
```ts twoslash import.meta.vitest name="handling-multiple-tagged-errors-1"
...
new FooError({ message: "Oh no!" })._tag // => "Foo"
```
