...
Then make the highlighted changes to our Express app:
...
The examples below are intentionally simple string manipulations to demonstrate
how to use the `Effect` type.
</Aside>
...
If we want to run these Effects, we can use `Effect.runSync` and
`Effect.runPromise`:
...
it a lot. Another thing you'll see used a lot is `Effect.pipe`. Here's the same
code as above, but using `Effect.pipe`:
...
This output has 2 small differences from the Express version:
