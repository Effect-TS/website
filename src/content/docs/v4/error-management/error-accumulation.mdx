...
The `Effect.validateFirst` function is similar to `Effect.validate` but it returns the first successful result, or all errors if none succeed.
...
Notice that `Effect.validateFirst` returns a single `number` as the success type, rather than an array of results like `Effect.validate`.
