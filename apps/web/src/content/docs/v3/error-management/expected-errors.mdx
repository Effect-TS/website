...
Expected errors are tracked at the type level by the [Effect data type](/docs/v3/getting-started/the-effect-type/) in the "Error channel":
...
For more information on constructing tagged errors, see [Data.TaggedError](/docs/v3/error-management/yieldable-errors/#datataggederror).
...
The `Effect.either` function transforms an `Effect<A, E, R>` into an effect that encapsulates both potential failure and success within an [Either](/docs/v3/data-types/either/) data type:
...
Transforms an effect to encapsulate both failure and success using the [Option](/docs/v3/data-types/option/) data type.
...
<Aside type="note" title="Recoverable Errors Only">
  `Effect.catchAll` only handles recoverable errors. It will not recover from
  unrecoverable defects. See [Effect.catchAllCause](#catchallcause) for handling
...
<Aside type="caution" title="Error Type Requirement">
  The error type must have a readonly `_tag` field to use `catchTag`. This field
  is used to identify and match errors.
</Aside>
...
- **Automatic span creation** for [tracing](/docs/v3/observability/tracing/) when a span name is provided.
...
`Effect.fn` automatically creates [spans](/docs/v3/observability/tracing/). The spans capture information about the function execution, including metadata and error details.
