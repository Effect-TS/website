...
```ts filename="withDefault.ts" /Config.withDefault(8080)/ file=<rootDir>/src/guide/configuration/withDefault-gen.ts
...
```ts filename="withDefault.ts" /Config.withDefault(8080)/ file=<rootDir>/src/guide/configuration/withDefault-pipe.ts
