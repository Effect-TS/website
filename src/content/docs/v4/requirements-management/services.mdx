...
class Random extends Context.Service<Random, { readonly next: Effect.Effect<number> }>()(
  "MyRandomService",
) {}
...
await Effect.runPromise(Effect.provideService(program, Random, { next: Effect.succeed(42) })) // => undefined
```
...
To retrieve the service type from a tag, use the `Context.Service.Shape` utility type.
...
await Effect.runPromise(
  program.pipe(
    ...
    Effect.provideService(Logger, { log: (message) => Effect.sync(() => console.log(message)) }),
  ),
) // => undefined
```
