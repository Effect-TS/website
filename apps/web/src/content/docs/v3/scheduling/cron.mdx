...
It also supports partial constraints (e.g., certain months or weekdays), time zone awareness through the [DateTime](/docs/v3/api/effect/DateTime) module, and robust error handling.
...
The `parse(cronExpression, tz?)` function safely parses a cron string into a `Cron` instance. It returns an [Either](/docs/v3/api/effect/Either), which will contain either the parsed `Cron` or a parsing error.
...
The `unsafeParse(cronExpression, tz?)` function works like [parse](#parse), but instead of returning an [Either](/docs/v3/api/effect/Either), it throws an exception if the input is invalid.
...
The `match` function allows you to determine if a given `Date` (or any [DateTime.Input](/docs/v3/api/effect/DateTime)) satisfies the constraints of a cron schedule.
