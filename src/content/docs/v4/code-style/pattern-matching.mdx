...
The `Match.result` method wraps the result in an [Either](/docs/data-types/either/), providing a structured way to distinguish between matched and unmatched cases. If a match is found, it returns `Success(value)`, otherwise, it returns `Failure(no match)`.

**Example** (Extracting a User Role with Either)

```ts twoslash import.meta.vitest name="extracting-a-user-role-with-either-1"
import { Match } from "effect"
...
console.log(getRole({ role: "admin" }))
// Output: { _id: 'Result', _tag: 'Success', value: 'Has full access' }

console.log(getRole({ role: "viewer" }))
// Output: { _id: 'Result', _tag: 'Failure', failure: { role: 'viewer' } }
```
