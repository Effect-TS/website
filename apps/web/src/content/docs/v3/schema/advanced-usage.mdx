...
When defining a new data type, some compilers like [Arbitrary](/docs/v3/schema/arbitrary/) or [Pretty](/docs/v3/schema/pretty/) may not know how to handle the new type.
...
For more details on how to add annotations for the Arbitrary compiler, refer to the [Arbitrary](/docs/v3/schema/arbitrary/) documentation.
...
- have already defined a branded type via [`effect/Brand`](/docs/v3/code-style/branded-types/) and want to reuse it to define a schema
...
If you have already defined a branded type using the [`effect/Brand`](/docs/v3/code-style/branded-types/) module, you can reuse it to define a schema using the `Schema.fromBrand` function.
...
When working with optional fields, you may want to handle them as [Option](/docs/v3/data-types/option/) types. This approach allows you to explicitly manage the presence or absence of a field rather than relying on `undefined` or `null`.
