...
In the [previous section](services.mdx), you learned how to create effects which depend on some context to be provided in order to execute, as well as how to provide that context to an Effect.
...
We will use `Layer`s to construct the `Recipe` service instead of providing a service implementation directly as we did in the [Managing Services](services.mdx) guide.
...
Just like in the [Managing Services](services.mdx) guide, we must create a `Tag` for the service.
And because the service has no dependencies, we can create the layer directly using `Layer.succeed(tag, service){:ts}`:

```ts filename="MeasuringCup.ts" file=<rootDir>/src/context-management/layers/MeasuringCup.ts
...
Just like we did in the [Services](services.mdx#using-the-service) guide, we can map over the `MeasuringCup` Tag to "extract" the service from the context and make use of it within our ingredient services.
...
```ts filename="Ingredients.ts" file=<rootDir>/src/context-management/layers/Ingredients.ts
...
```ts filename="Recipe.ts" file=<rootDir>/src/context-management/layers/Recipe-gen.ts
...
```ts filename="Recipe.ts" file=<rootDir>/src/context-management/layers/Recipe.ts
...
```ts filename="index.ts" file=<rootDir>/src/context-management/layers/index.ts
...
```ts filename="index.ts" file=<rootDir>/src/context-management/layers/index2.ts
...
```ts file=<rootDir>/src/context-management/layers/index2-use.ts#L9-
...
```ts filename="index.ts" file=<rootDir>/src/context-management/layers/index2-provideMerge.ts

```

Alternatively, you can achieve the same result in a bottom-up fashion by using `Layer.useMerge`:

```ts filename="index.ts" file=<rootDir>/src/context-management/layers/index2-useMerge.ts
...
```ts filename="index.ts" {28} file=<rootDir>/src/context-management/layers/index-gen.ts
...
```ts filename="index.ts" {27} file=<rootDir>/src/context-management/layers/index-pipe.ts
