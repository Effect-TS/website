...
```ts twoslash import.meta.vitest name="simulating-a-coin-flip-1"
import { Effect, Random, Console } from "effect"

const flipTheCoin = Random.nextBoolean.pipe(
  Effect.andThen((isHead) => (isHead ? Console.log("Head") : Console.log("Tail"))),
)
...
```ts twoslash import.meta.vitest name="conditional-effect-execution-1"
import { Effect, Option } from "effect"

const validateWeightOption = (weight: number): Effect.Effect<Option.Option<number>> =>
  // Conditionally execute the effect if the weight is non-negative
  Effect.succeed(weight).pipe(Effect.when(Effect.succeed(weight >= 0)))
...
```ts twoslash import.meta.vitest name="effectful-iteration-1"
import { Effect } from "effect"
...
If any effect fails, it stops execution (short-circuiting) and propagates the error. To change this behavior, you can use the [`mode`](#the-mode-option) option, which allows all effects to run and collect results as [Result](/docs/data-types/result/) or [Option](/docs/data-types/option/).
...
The `{ mode: "either" }` option changes the behavior of `Effect.all` to ensure all effects run, even if some fail. Instead of stopping on the first failure, this mode collects both successes and failures, returning an array of `Result` instances where each result is either a `Success` (success) or a `Failure` (failure).

**Example** (Collecting Results with `mode: "either"`)

```ts twoslash /{ mode: "result" }/ import.meta.vitest name="collecting-results-with-mode-either-1"
import { Effect, Console } from "effect"
...
Task3
{
  ...
  _tag: 'Success',
  value: [
    { _id: 'Result', _tag: 'Success', value: 'Task1' },
    { _id: 'Result', _tag: 'Failure', failure: 'Task2: Oh no!' },
    { _id: 'Result', _tag: 'Success', value: 'Task3' }
  ]
}
*/
```
