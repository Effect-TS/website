...
```ts {6} file=<rootDir>/src/data-types/option/optional-property.ts#L1-L7
...
```ts file=<rootDir>/src/data-types/option/optional-property.ts#L9-
...
```ts file=<rootDir>/src/data-types/option/guards.ts
...
```ts file=<rootDir>/src/data-types/option/match.ts
...
```ts file=<rootDir>/src/data-types/option/map-some.ts

```

The convenient aspect of using `Option` is how it handles the absence of a value, represented by `None`:

```ts file=<rootDir>/src/data-types/option/map-none.ts
...
```ts {7} file=<rootDir>/src/data-types/option/nested.ts#L1-L8

```

The `address` field itself contains a nested optional property called `street` of type `Option<string>`:

```ts {3} file=<rootDir>/src/data-types/option/nested.ts#L10-L13
interface Address {
...
```ts file=<rootDir>/src/data-types/option/nested.ts#L15-
...
- `getOrThrow`: It retrieves the wrapped value from an `Option`, or throws an error if the `Option` is a `None`. Here's an example:
  ...
  ```ts file=<rootDir>/src/data-types/option/getOrThrow.ts
...
```ts file=<rootDir>/src/data-types/option/orElse.ts
...
```ts file=<rootDir>/src/data-types/option/firstSomeOf.ts
...
```ts file=<rootDir>/src/data-types/option/fromNullable.ts
...
```ts file=<rootDir>/src/data-types/option/interop-effect.ts
...
```ts file=<rootDir>/src/data-types/option/zipWith.ts
...
```ts {4} file=<rootDir>/src/data-types/option/zipWith-none.ts
