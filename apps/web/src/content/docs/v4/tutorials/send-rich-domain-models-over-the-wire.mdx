...
<TutorialArchitectureMap
  active="send-rich-domain-models-over-the-wire"
  compact
/>

## Project setup

The example uses three small files:

- `order.ts` defines the accepted-order model and a fixture that returns one;
- `server.ts` is the HTTP boundary and changes as each JSON strategy is added;
- `client.ts` represents a client owned by the same team and appears when the
  shared Schema is ready.

Install the Effect version used by these examples:
...
newer. Each checkpoint is a short summary of the behavior reached so far.

The server can run with `--watch`, which reloads `server.ts` and its imports
after every edit. Keep this command active in one terminal pane:
