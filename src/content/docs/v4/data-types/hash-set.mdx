...
const modified = original.pipe(HashSet.add(4), HashSet.add(5), HashSet.remove(1))
...
In v4, plain objects implement [`Equal`](/docs/trait/equal/) automatically, based on structural equality — no wrapper needed. Values decoded with `Schema` already benefit from this, as shown below.
...
// (no Data wrapper needed - v4 gives decoded objects structural
...
`values`, `toValues`, `toggle`, `flatMap`, `forEach`, and `partition` were removed in v4. `HashSet` is directly `Iterable`, so use `Array.from(self)` or `self[Symbol.iterator]()` for values; compose `flatMap`/`forEach`/`partition` from the `Iterable` module or two `filter` calls instead.
...
//      ┌─── Array<number>
//      ▼
const array = Array.from(hashSet)
