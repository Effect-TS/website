...
1. **Legacy API-reference redirects were removed.** `/docs/api/v4`, package,
   and module URLs return `404` on the PR Worker instead of redirecting to their
   `/docs/v4/api` equivalents.
2. **Generated Open Graph cache keys are incomplete.** Most generated image URLs
   ...
   metadata.
3. **Playground filesystem acknowledgements can become stale.** Completed write
...
4. **Expired Mixedbread stores can be treated as healthy.** The provider
