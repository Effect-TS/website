...
| `logLevel` | Reads a value as a [LogLevel](/docs/v3/observability/logging/#log-levels). |
...
For more information, see the [Schema.Config](/docs/v3/schema/effect-data-types/#config) documentation.
...
It parses the configuration value and wraps it in a `Redacted<string>`, a specialized [data type](/docs/v3/data-types/redacted/) designed to protect secrets.
...
<Aside type="tip">
  In this example, we use `ConfigProvider.fromMap` to create a configuration
  provider from a simple JavaScript `Map`. This is particularly useful for
  testing, as described in the [Mocking Configurations in
