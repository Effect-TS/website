...
<Aside type="caution" title="Risk of Mutable Data">
  Using `Chunk.unsafeFromArray` can lead to unexpected behavior if the original
...
To check if two `Chunk` instances are equal, use [`Equal.equals`](/docs/v3/trait/equal/). This function compares the contents of each `Chunk` for structural equality.
