...
The `Result` type is a streamlined version of the [Exit](../data-types/exit) type, designed to capture the outcome of a `Micro` computation. It uses the [Either](./data-types/either) data type to distinguish between successful outcomes and failures:
...
The `Failure` type is a streamlined version of the [Cause](../data-types/cause) type.

Similar to how `Cause` is a union of types, `Failure` consists of three specific types:

```ts
type Failure<E> = Unexpected | Expected<E> | Aborted
```

| **Failure Type** | **Similar to** | **Description**                                                                             |
| ---------------- | -------------- | ------------------------------------------------------------------------------------------- |
| `Unexpected`     | `Die`          | Indicates an unforeseen defect that wasn't planned for in the system's logic.               |
| `Expected<E>`    | `Fail<E>`      | Covers anticipated errors that are recognized and typically handled within the application. |
| `Aborted`        | `Interrupt`    | Signifies an operation that has been purposefully stopped.                                  |

### DelayFn

The `DelayFn` type is a streamlined version of the [Schedule](../../guides/scheduling/introduction) type.

```ts
type DelayFn = (attempt: number, elapsed: number) => Option<number>
```
