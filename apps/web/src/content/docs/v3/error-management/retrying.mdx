...
The `Effect.retry` function takes an effect and a [Schedule](/docs/v3/scheduling/introduction/) policy, and will automatically retry the effect if it fails, following the rules of the policy.
...
The `Effect.retryOrElse` function attempts to retry a failing effect multiple times according to a defined [Schedule](/docs/v3/scheduling/introduction/) policy.
