...
In some cases, you may need to partition a stream using an effectful function of type `(a: A) => Effect<R2, E2, readonly [K, V]>`, where the partitioning operation itself involves effects. For such scenarios, the `groupBy` function provides a more powerful solution.

The `groupBy` function takes an effectful partitioning function and creates a new data type called `GroupBy`. This `GroupBy` type represents a grouped stream. To work with the groups, you can use the `evaluate` function, which takes a function of type `(key: K, stream: Stream<never, E, V>) => Stream.Stream<R2, E2, A>`. This function runs across all groups and merges them in a non-deterministic fashion.
