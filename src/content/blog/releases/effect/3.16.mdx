...
const withPlan: Effect.Effect<void> = Effect.withExecutionPlan(effect, AiPlan)
```
...
const CoolNumberServiceLayer: Layer.Layer<NumberService> = NumberService.Default(6942)
```
...
const Port = Brand.refined<Port>(
  (num) => !Number.isNaN(num) && Number.isInteger(num) && num >= 1 && num <= 65535,
