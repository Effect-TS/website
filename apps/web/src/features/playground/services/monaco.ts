...
function setupCompletionItemProviders(monaco: MonacoApi) {
  ...
  monaco.languages.registerCompletionItemProvider = function (
    ...
    async function provideCompletionItems(
      ...
      const info: MonacoWorkerCompletionInfo | undefined =
    ...
    ],
    {
      ...
      provideInlayHints: async (model, _, cancellationToken) => {
        ...
        while ((match = queryRegex.exec(text)) !== null) {
          ...
          const quickInfo: MonacoWorkerQuickInfo | undefined =
...
async function yieldTypeScriptLikeWorker(
  monaco: MonacoApi,
  model: monaco.editor.ITextModel,
) {
  ...
  try {
    ...
    if (
      error === "TypeScript not registered!" ||
      error === "JavaScript not registered!"
    ) {
      return undefined
    }
    throw error
