...
<Callout>
  ...
  is _after_ the call to `Random.next()` in the `pipe`, so `Random.next()` is passed in last (i.e. `data-last`). Similarly, you can
  also call `Effect.flatMap(Random.next(), (n) => ...)` directly without `pipe` with `Random.next()` as the first argument (i.e. `data-first`) get the same result.
</Callout>
