...
Alternatively, you can use the Class APIs (see the [Class APIs](/docs/v3/schema/classes/) section for more details).
...
| `decodeUnknownOption`  | Decodes a value and returns an [Option](/docs/v3/api/effect/Option) type.           |
| `decodeUnknownEither`  | Decodes a value and returns an [Either](/docs/v3/api/effect/Either) type.           |
| `decodeUnknownPromise` | Decodes a value and returns a `Promise`.                                            |
| `decodeUnknown`        | Decodes a value and returns an [Effect](/docs/v3/getting-started/the-effect-type/). |
...
The `Schema.decodeUnknownEither` function allows you to parse a value and receive the result as an [Either](/docs/v3/api/effect/Either), representing success (`Right`) or failure (`Left`). This approach lets you handle parsing errors more gracefully without throwing exceptions.
...
In such cases, you should use the `Schema.decodeUnknown` function, which returns an [Effect](/docs/v3/getting-started/the-effect-type/).
...
| `encodeOption`  | Encodes data and returns an [Option](/docs/v3/api/effect/Option) type.                                 |
| `encodeEither`  | Encodes data and returns an [Either](/docs/v3/api/effect/Either) type representing success or failure. |
| `encodePromise` | Encodes data and returns a `Promise`.                                                                  |
| `encode`        | Encodes data and returns an [Effect](/docs/v3/getting-started/the-effect-type/).                       |
...
Here is an example of a transformation that never fails during decoding. It returns an [Either](/docs/v3/api/effect/Either) containing either the decoded value or the original input. For encoding, it is reasonable to not support it and use `Forbidden` as the result.
...
- **Decoding**: The `SafeDecode` function ensures that decoding never fails. It wraps the decoded value in an [Either](/docs/v3/api/effect/Either), where a successful decoding results in a `Right` and a failed decoding results in a `Left` containing the original input.
...
The `Schema.decodeUnknownEither` and `Schema.encodeEither` functions returns a [Either](/docs/v3/api/effect/Either):
...
It is wrapped in a tagged error to make it easier to catch errors using [Effect.catchTag](/docs/v3/error-management/expected-errors/#catchtag).
