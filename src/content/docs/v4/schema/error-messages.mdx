...
// Output: SchemaError: Expected object
```
...
const schema = Schema.String.pipe(
  Schema.decodeTo(Schema.String.check(Schema.isMinLength(2)), {
    decode: SchemaGetter.transformOrFail((s) =>
      s.length > 0 ? Effect.succeed(s) : Effect.fail(new SchemaIssue.InvalidValue()),
    ),
...
const MyUnionWithMessage = Schema.Union([Schema.String, Schema.Number]).annotate({
...
This example demonstrates setting a custom message on the last refinement in a chain of refinements. As you can see, the custom message is only used if the refinement related to `isMaxLength` fails; otherwise, default messages are used.
...
const MyString = Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(2)).annotate({
        ...
        decode: SchemaGetter.transformOrFail((s) => {
          ...
          return Number.isNaN(n)
            ? Effect.fail(
                ...
                new SchemaIssue.InvalidValue({ message: "please enter a parseable string" }),
              )
            : Effect.succeed(n)
        }),
