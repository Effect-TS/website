...
| Equivalence     | Create [Equivalence](/docs/v3/schema/equivalence/) based on defined schemas.         |
...
| **Requirements** | Similar to the [`Effect`](/docs/v3/getting-started/the-effect-type) type, it represents the contextual data required by the schema to execute both decoding and encoding. If this type parameter is `never` (default if not explicitly provided), it means the schema has no requirements. |
...
<Aside type="note" title="Type Parameter Abbreviations">
  ...
  `Schema` abbreviated as `A`, `I`, and `R` respectively. This is just shorthand
  for the type value of type **A**, **I**nput, and **R**equirements.
</Aside>
...
<Aside type="tip" title="Ensure Consistency">
  As a general rule, schemas should be defined such that encode + decode return
  the original value.
</Aside>
