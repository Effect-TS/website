...
As you saw above, an `Effect` can name several expected errors in the `Error` type
parameter using a union. At the type level, this tells the caller every failure
it may need to handle. At runtime, the program fails with one specific error.
The caller needs to identify that error to decide what to do, such as retrying a
network request or asking for a different card.

Effect provides `Data.TaggedError` for this purpose. When you define an error,
you choose an identifier that is unique across your application. This ensures
that the error can be distinguished from every other error:

{/* prettier-ignore */}
```ts import.meta.vitest showLineNumbers=false
...
`Data.TaggedError` stores this identifier in a `_tag` property. Later, Effect
uses `_tag` to select the matching error handler.
