...
<Aside type="note">
  ...
  change the overall success or failure of the effect. It only transforms the
  values in the error channel while preserving the effect's original success or
  failure status.
</Aside>
...
If you prefer, you can utilize a pre-made guard like [Predicate.isNotNull](/docs/v3/api/effect/Predicate#isNotNull) for simplicity and consistency.

## Inspecting Errors

Similar to [tapping](/docs/v3/getting-started/building-pipelines/#tap) for success values, Effect provides several operators for inspecting error values.
...
Specifically inspects non-recoverable failures or defects in an effect (i.e., one or more [Die](/docs/v3/data-types/cause/#die) causes).
...
This function becomes especially useful when recovering from effects that may fail when using [Effect.gen](/docs/v3/getting-started/using-generators/#understanding-effectgen):
