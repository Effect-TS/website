...
<Warning>
  Please note that the documentation is currently under construction and some sections may be incomplete or subject to changes. We are working hard to
...
In these documentation pages, we use the `$ExpectType` pragma borrowed from [dtslint](https://github.com/microsoft/DefinitelyTyped-tools/tree/master/packages/dtslint) to display the type of a variable. Here's an example:
