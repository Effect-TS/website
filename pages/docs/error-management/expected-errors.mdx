...
As we saw in the guide [Creating Effects](../essentials/creating.mdx), we can use the `fail` constructor to create an Effect
...
The `Effect.either` function converts an `Effect<A, E, R>` into another effect where both its failure (`E`) and success (`A`) channels have been lifted into an [`Either<E, A>`](../data-types/either.mdx) data type:

```ts
Effect<A, E, R> -> Effect<R, never, Either<E, A>>
```

The [`Either<E, A>`](../data-types/either.mdx) data type represents a value that can be either an error (`E`) or a successful result (`A`).
