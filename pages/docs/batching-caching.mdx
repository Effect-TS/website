...
Batching can be locally disabled using the `Effect.withRequestBatching(false){:ts}` utility in the following way:

```ts {12} filename="index.ts" file=<rootDir>/src/guide/batching-caching/withRequestBatching.ts
