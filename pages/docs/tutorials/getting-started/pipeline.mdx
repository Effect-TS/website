...
assert.equal(Effect.runSync(mappedEffect), 6)
```
...
assert.equal(Effect.runSync(flatMappedEffect), 5)
```
...
const divideByTen = (x: number): Effect.Effect<never, never, number> =>
  Effect.succeed(x / 10)
...
const result = pipe(
  ...
  multiplyByTwo,
  Effect.succeed,
  Effect.flatMap(divideByTen),
  Effect.map((x) => `Result: ${x}`)
)

assert.equal(Effect.runSync(result), "Result: 1.2")
```

In this example, we start with the initial value of 5. We use `pipe` to chain the transformations:

- `addOne` adds 1
- `multiplyByTwo` multiplies by 2
- `succeed` lifts the value into an `Effect`
- `divideByTen` divides by 10 using an `Effect` transformation
