...
```ts twoslash import.meta.vitest name="defining-a-tool-1"
...
```ts twoslash import.meta.vitest name="creating-a-toolkit-1"
...
Object.keys(DadJokeTools.tools) // => ["GetDadJoke"]
```
...
```ts twoslash /DadJokeTools.toLayer/ collapse={10-45} import.meta.vitest name="implementing-a-toolkit-1"
import { Tool, Toolkit } from "effect/unstable/ai"
...
class ICanHazDadJoke extends Context.Service<ICanHazDadJoke>()("ICanHazDadJoke", {
    ...
    const search = Effect.fn("ICanHazDadJoke.search")(function* (searchTerm: string) {
      return yield* httpClientOk
        ...
        .pipe(
          ...
          Effect.flatMap(({ results }) => Effect.fromOption(Array.head(results))),
...
```ts twoslash {19} import.meta.vitest name="using-a-toolkit-1"
...
```ts twoslash /DadJokeToolHandlers/ collapse={11-64}
import { LanguageModel, Tool, Toolkit } from "effect/unstable/ai"
import { OpenAiClient, OpenAiLanguageModel } from "@effect/ai-openai"
import { HttpClient, HttpClientRequest, HttpClientResponse } from "effect/unstable/http"
import { NodeHttpClient } from "@effect/platform-node"
import { Array, Config, Console, Context, Effect, Layer, Schema } from "effect"
...
).pipe(Layer.provide(ICanHazDadJoke.layer))
