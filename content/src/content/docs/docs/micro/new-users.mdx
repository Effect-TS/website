...
/*
throws:
DieImpl [(MicroCause.Die) Error]: Cannot divide by zero
*/
```
...
//      ┌─── Micro<Fiber<number, never>, never, never>
//      ▼
...
const program = Micro.gen(function* () {
  ...
  // TODO: doesn't return exit anymore
  // Interrupt the fiber and get the MicroExit result
  const exit = yield* Micro.fiberInterrupt(fiber)
  console.log(exit)
})
