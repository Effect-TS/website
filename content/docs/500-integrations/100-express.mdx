...
- **Express Service**. We define an `Express` [service](context-management/services) to retrieve the Express app later on.
...
- **Main Route**. The main route, `IndexRouteLive`, is defined as a [Layer](context-management/layers).
  ...
  We access the [runtime](runtime) (`Effect.runtime`), which can be used to execute tasks within our route (`runFork`).
