...
```ts filename="primitives.ts" file=<rootDir>/src/tutorial/configuration/primitives-pipe.ts
...
```ts filename="primitives.ts" file=<rootDir>/src/tutorial/configuration/primitives-gen.ts
...
```ts filename="withDefault.ts" /Config.withDefault(8080)/ file=<rootDir>/src/tutorial/configuration/withDefault.ts
...
```ts filename="HostPort.ts" file=<rootDir>/src/tutorial/configuration/HostPort.ts#L3-L9

```

We can define a configuration for this data type by combining primitive configs for `string` and `number`:

```ts filename="HostPort.ts" file=<rootDir>/src/tutorial/configuration/HostPort.ts
...
```ts filename="App.ts" {5} file=<rootDir>/src/tutorial/configuration/App.ts
...
```ts filename="App.ts" {6} file=<rootDir>/src/tutorial/configuration/App-gen.ts
...
```ts filename="ServiceConfig.ts" file=<rootDir>/src/tutorial/configuration/ServiceConfig.ts
...
```ts /Config.nested/ filename="ServiceConfig.ts" file=<rootDir>/src/tutorial/configuration/ServiceConfig-nested.ts#L8-
...
```ts filename="mockConfigProvider.ts" file=<rootDir>/src/tutorial/configuration/mockConfigProvider.ts
