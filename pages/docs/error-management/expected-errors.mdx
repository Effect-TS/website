...
If all the errors in our program are tagged with a `_tag` field as a discriminant, we can use the `Effect.catchTag` function to catch and handle specific errors.
