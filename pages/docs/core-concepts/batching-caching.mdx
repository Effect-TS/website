...
const getUserById = (id: number) => Effect.tryCatchPromise(
  () => fetch("https://api.example.demo/todos").then(_ => _.json() as Promise<User>),
  () => new GetUserError()
)
...
<Callout type="info">
In a real world scenario we may not want to trust our APIs to actually return the expected data, for doing this properly you can use `@effect/schema` or similar alternatives such as `zod`.
</Callout>
...
const resolveGetTodos = RequestResolver.fromFunctionEffect((request: GetTodos) =>
...
const resolveGetUserById = RequestResolver.makeBatched((requests: GetUserById[]) =>
  pipe(
    Effect.tryCatchPromise(
...
const resolveSendEmail = RequestResolver.makeBatched((requests: SendEmail[]) =>
  pipe(
    Effect.tryCatchPromise(
...
const main = Effect.provideSomeLayer(GetUserByIdCacheLive)(
...
const nextStep = Effect.flatMapStep(program, (step) => {
  switch (step._tag) {
    ...
    case "Blocked": {
      ...
      return Effect.flatMap(shipRequestsToBeExecutedAndWait(requests), () => continuation)
    }
