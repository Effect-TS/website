...
const createDocsTemplate = (
  ...
  const { fontSize, lineHeight, maxWidth } = getDocsTitleStyles(props.title)
...
const getDocsTitleStyles = (
  ...
  const lineHeight = length > 64 ? 1.14 : 1.08
  const maxWidth = length <= 52 ? "920px" : "860px"
