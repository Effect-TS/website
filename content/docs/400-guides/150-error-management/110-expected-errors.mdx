...
The `Effect.either` function converts an `Effect<R, E, A>` into another effect where both its failure (`E`) and success (`A`) channels have been lifted into an [`Either<E, A>`](../data-types/either-effects) data type:

```ts
Effect<R, E, A> -> Effect<R, never, Either<E, A>>
```

The [`Either<E, A>`](../data-types/either-effects) data type represents a value that can be either an error (`E`) or a successful result (`A`).
