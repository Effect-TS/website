...
See MDN for more information:<br />
...
The `Redacted<T>` is a data type that represents sensitive data. It is generic, so it can
be used to represent any type of data.
...
Http.request.get(new URL("https://example.com/?foo=bar#hash")).pipe(
...
- `isTupleOf` - a refinement that ensures a value is a tuple of a specific length
- `isTupleOfAtLeast` - a refinement that ensures a value is a tuple at least the specified length
