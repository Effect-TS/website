...
- Creating a `Chunk` from a generic `Iterable`:
  ...
  ```ts file=<rootDir>/src/data-types/chunk/fromIterable.ts

  ```

- Creating a `Chunk` from an `Array`:

  ```ts file=<rootDir>/src/data-types/chunk/unsafeFromArray.ts
...
```ts file=<rootDir>/src/data-types/chunk/appendAll.ts
...
```ts file=<rootDir>/src/data-types/chunk/drop.ts
...
```ts file=<rootDir>/src/data-types/chunk/equals.ts
...
```ts file=<rootDir>/src/data-types/chunk/toReadonlyArray.ts
