...
type AuthorizationOutcome =
  ...
  | {
      readonly ok: false
      readonly error: ExpiredCardError | NetworkError | InvalidTokenError
    }
...
As you saw above, the `Error` type parameter can contain a union of expected
errors. For that union to be useful, TypeScript must be able to distinguish its
members. Class names alone are not enough because TypeScript compares types by
their structure.

Effect error classes use two conventions to keep errors distinct and
recognizable:
