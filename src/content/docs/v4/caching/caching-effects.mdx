...
`Effect.cachedFunction` was removed in v4 with no direct replacement. To memoize an effectful function, use the `Cache` module directly: create a cache whose `lookup` is the function to memoize, then call `Cache.get` for each input. The cache stores one result per input, so calling the wrapped function again with the same input reuses the cached result instead of recomputing it.
...
const program = Effect.gen(function* () {
  ...
  console.log(yield* randomNumber(10)) // Computes a new result
  console.log(yield* randomNumber(10)) // Computes a different result
  ...
  console.log(yield* memoized(10)) // Computes and caches the result
