...
```ts twoslash
import { pipe } from "@effect/data/Function";
import { Either, left, right, flatMap, map } from "@effect/data/Either";
...
const result = pipe(
  ...
  map((n) => 1) // ok! now `n` has type `number`
);
```
...
const failure: Either<string, number> = pipe(
  right("Not a number"),
  flatMap((s) => parseNumber(s)), // parse the input to number
  map((x) => x * 2), // This will not be executed because parseNumber will return Left
...
```ts twoslash
import { pipe } from "@effect/data/Function";
import { Either, right, left, orElse } from "@effect/data/Either";
