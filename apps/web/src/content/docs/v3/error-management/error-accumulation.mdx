...
Sequential combinators such as [Effect.zip](/docs/v3/getting-started/control-flow/#zip), [Effect.all](/docs/v3/getting-started/control-flow/#all) and [Effect.forEach](/docs/v3/getting-started/control-flow/#foreach) have a "fail fast" policy when it comes to error management. This means that they stop and return immediately when they encounter the first error.
...
<Aside type="caution" title="Loss of Successes">
  ...
  all successes will be lost. If you need to preserve both successes and
  failures, consider using [Effect.partition](#partition).
</Aside>
