...
Runs an effect synchronously and returns the result as an [Exit](/docs/v3/data-types/exit/) type, which represents the outcome (success or failure) of the effect.
...
- If it fails, the failure information is provided as a `Failure` containing
  a [Cause](/docs/v3/data-types/cause/) type.
...
Runs an effect and returns a `Promise` that resolves to an [Exit](/docs/v3/data-types/exit/), which
...
In this example, the `program` continuously logs "running..." with each repetition spaced 200 milliseconds apart. You can learn more about repetitions and scheduling in our [Introduction to Scheduling](/docs/v3/scheduling/introduction/) guide.
...
For a deeper understanding of how fibers work and how to handle interruptions, check out our guides on [Fibers](/docs/v3/concurrency/fibers/) and [Interruptions](/docs/v3/concurrency/basic-concurrency/#interruptions).
...
You can find the complete list of `run*` functions [here](/docs/v3/api/effect/Effect#category-running-effects).
