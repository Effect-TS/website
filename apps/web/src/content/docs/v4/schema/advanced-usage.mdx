...
const FileSchema = Schema.declare((input: unknown): input is File => input instanceof File)
...
export const MyReadonlySet = <S extends Schema.Constraint>(
  ...
  Schema.declareConstructor<ReadonlySet<S["Type"]>, ReadonlySet<S["Encoded"]>>()(
...
<Aside type="caution" title="Decoding/Encoding Limitations">
  The parser returned by `declareConstructor` is effectful, but the resulting codec can require only
  the decoding and encoding services already required by its type-parameter schemas.
</Aside>
