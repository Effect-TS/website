...
The [`Effect<R, E, A>`](../../guides/essentials/the-effect-type) type is polymorphic in values of type `E`, which means we can work with any error type we want. However, there is additional information related to failures that is not captured by the `E` value alone.
...
const effect: Effect.Effect<never, never, never> = Effect.failCause(
  Cause.empty
)
```
