...
Output:
{
  ...
  cause: {
    _id: 'Cause',
    failures: [ { _tag: 'Fail', error: 'Something went wrong' } ]
  }
}
*/
```
...
Conceptually, `Exit<A, E>` can be thought of as `Result<A, Cause<E>>`. However, the [Cause](/docs/data-types/cause/) type represents more than just expected errors of type `E`. It includes:
