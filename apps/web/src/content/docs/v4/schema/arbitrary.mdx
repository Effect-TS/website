...
<Aside type="tip" title="FastCheck API">
  Effect re-exports the complete fast-check API as `FastCheck` from `effect/testing`.
</Aside>
...
samples.every((value) => value.length >= 3 && value.length <= 20 && /^[a-z0-9_]+$/.test(value)) // => true
```
...
const isPalindrome = (value: string) => value === Array.from(value).reverse().join("")
...
Synthetic data libraries such as `@faker-js/faker` can provide realistic values. Seed them from a fast-check arbitrary so runs remain reproducible and values can shrink.

**Example** (Using Faker from a toArbitrary Annotation)
...
function fake<T>(
  generate: (instance: typeof faker) => T,
): Schema.Annotations.ToArbitrary.Declaration<T, readonly []> {
  return () => (fc) =>
    fc.nat().map((seed) => {
      faker.seed(seed)
