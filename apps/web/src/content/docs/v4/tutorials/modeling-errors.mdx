...
- **Over-specification**: TypeScript compiles even if `authorizePayment` never throws it.
- **Under-specification**: forgetting to handle an error in `processPayment` that `authorizePayment` does throw still compiles.
- **Drift**: when `authorizePayment` starts throwing a new error later, `processPayment` still compiles, and the error can reach production.
