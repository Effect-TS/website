...
export interface AtomWorkspaceHandle extends Atom.Success<
  ReturnType<typeof workspaceHandleAtom>
> {}
...
function setupWorkspaceTypeAcquisition(workspace: Workspace) {
  return Effect.gen(function* () {
          ...
          .pipe(
            Effect.map((entries) =>
              Array.partition(entries, (entry) =>
                entry.isDirectory()
                  ? Result.succeed(entry.name)
                  : Result.fail(entry.name),
              ),
            ),
          )
    ...
    const path: string = Option.getOrThrow(
...
class FormatterPluginLoadError extends Data.TaggedError(
  "FormatterPluginLoadError",
)<{
...
class FormatterPluginUrlError extends Data.TaggedError(
  "FormatterPluginUrlError",
)<{
  readonly plugin: string
}> {
  override get message(): string {
...
function setupWorkspaceFormatters(workspace: Workspace) {
  return Effect.gen(function* () {
    ...
    function loadPlugin(
      ...
      FormatterPlugin,
      FormatterPluginUrlError | FormatterPluginLoadError
    > {
    ...
    function loadPlugins(plugins: Array<string>) {
      return Effect.forEach(
        plugins,
        (plugin) => Cache.get(pluginCache, plugin),
        {
    ...
    function reconcilePlugins(plugins: Array<FormatterPlugin>) {
      return Effect.sync(() => {
        const desiredFormatters = new Map(
          plugins.map((plugin) => [plugin.language, plugin]),
        )

        installedFormatters.forEach((installed, language) => {
          if (
            desiredFormatters.get(language)?.formatter !== installed.formatter
          ) {
            installed.dispose()
