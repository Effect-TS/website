...
export type PluginMonacoEditorFeatures =
  | "all"
  | ReadonlyArray<EditorFeature | `!${EditorFeature}`>
...
function resolveLanguages(
  ...
  ;(metadata.languages as ReadonlyArray<IFeatureDefinition>).forEach(
    (language) => {
...
function resolveFeatures(features: PluginMonacoEditorFeatures) {
  ...
  ;(metadata.features as ReadonlyArray<IFeatureDefinition>).forEach(
    (feature) => {
...
function resolveMonacoPath(file: string) {
  ...
  try {
    return resolveModule(
      path.join(process.cwd(), "node_modules/@effect/monaco-editor/esm", file),
    )
  } catch {}
...
export function monacoEditorPlugin(
  options: PluginMonacoEditorOptions = {},
): Plugin {
  const {
    globalAPI = false,
    languages = [],
    ...
    load(id) {
      if (id.match(/esm[/\\]vs[/\\]editor[/\\]editor.main.js/)) {
        ...
        const workerImports = resolvedWorkers.map(
          (worker) =>
            `import ${worker.label} from "${resolveMonacoPath(worker.entry)}?worker"`,
        )
