...
const resolveGetTodos = RequestResolver.fromFunctionEffect<GetTodos>()((request) =>
...
const resolveGetUserById = RequestResolver.makeBatched<GetUserById>()((requests) =>
  pipe(
    Effect.tryCatchPromise(
...
const resolveSendEmail = RequestResolver.makeBatched<SendEmail>()((requests) =>
  pipe(
    Effect.tryCatchPromise(
...
const GetUserByIdCache = Context.Tag<GetUserByIdCache, Request.Cache<GetUserById>>(
  // this is not strictly needed but makes sure the tag is the same if module is reloaded
