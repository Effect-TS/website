...
The `Schema.toArbitrary` function allows for the creation of random values that align with a specific `Schema<A, I, R>`.
...
const Person = Schema.Struct({
  ...
  age: Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 80 })),
})
...
Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 80 }))
```
...
To generate efficient arbitraries for strings that must match a certain pattern, use the `Schema.isPattern` filter instead of writing a custom filter:

**Example** (Using `Schema.isPattern` for Pattern Constraints)

```ts twoslash import.meta.vitest name="patterns-1"
import { Schema } from "effect"

// ❌ Without using Schema.isPattern (less efficient)
const Bad = Schema.String.check(Schema.makeFilter((s) => /^[a-z]+$/.test(s)))

// ✅ Using Schema.isPattern (more efficient)
const Good = Schema.String.check(Schema.isPattern(/^[a-z]+$/))
```

By using `Schema.isPattern`, arbitrary generation will rely on `FastCheck.stringMatching(regexp)`, which is more efficient and directly aligned with the defined pattern.
...
```ts twoslash import.meta.vitest name="transformations-and-arbitrary-generation-1"
import { Schema } from "effect"
import { FastCheck } from "effect/testing"

// Schema with filters before the transformation
const schema1 = Schema.NonEmptyString.pipe(Schema.decodeTo(Schema.Trim)).check(
  Schema.isMaxLength(500),
)

// May produce empty strings due to ignored NonEmpty filter
console.log(FastCheck.sample(Schema.toArbitrary(schema1), 2))
/*
Example Output:
[ '', '"Ry' ]
*/
...
const schema2 = Schema.Trim.check(Schema.isNonEmpty(), Schema.isMaxLength(500))

// Adheres to all filters, avoiding empty strings
console.log(FastCheck.sample(Schema.toArbitrary(schema2), 2))
/*
Example Output:
[ ']H+MPXgZKz', 'SNS|waP~\\' ]
*/
...
- `schema1`: Takes into account `Schema.isMaxLength(500)` since it is applied after the `Schema.Trim` transformation, but ignores the `Schema.NonEmptyString` as it precedes the transformations.
...
```ts twoslash import.meta.vitest name="best-practices-1"
import { Schema, SchemaTransformation } from "effect"

// Recommended: Separate transformations and filters
const improved = Schema.String.pipe(
  Schema.decodeTo(
    Schema.String.check(Schema.isTrimmed(), Schema.isLowercased()),
...
You can customize how arbitrary data is generated using the `toArbitrary` annotation in schema definitions.
...
const Name = Schema.NonEmptyString.annotate({
  toArbitrary: () => (fc) => fc.constantFrom("Alice Johnson", "Dante Howell", "Marta Reyes"),
})

const Age = Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 80 }))
