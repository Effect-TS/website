...
const content = {
  ...
  survey: {
    ...
    heading: 'Most desired JavaScript features',
    subheading: 'According to the 2022 State of JavaScript survey',
    features: [
        ...
        <div className="flex gap-6">
          <div className="grow">
            ...
            <ul className="relative mt-6 space-y-3">
              ...
              {content.survey.features.map(({name, value}, index) => (
                <li key={index} className="relative" style={{width: `${(value / content.survey.features[0].value) * 100}%`}}>
                  <div className="absolute inset-0 bg-zinc-700 rounded-r-md" style={{opacity: `${100 - index * 15}%`}} />
                  <div className="relative items-center text-sm text-white px-2 w-full flex h-6">{name}</div>
                </li>
              ))}
            </ul>
          </div>
          <div className="shrink-0 bg-gradient-to-br from-zinc-300 to-zinc-500 p-px rounded-2xl">
