...
```ts
expect(Chunk.difference(Chunk.make(1, 2, 3, 4, 5), Chunk.make(1, 2, 3))).toEqual(Chunk.make(4, 5))
```
...
```ts
Effect.log("Hello World").pipe(Effect.withSpan("my span", { captureStackTrace: false }))
```
