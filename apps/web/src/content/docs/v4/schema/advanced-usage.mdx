...
Schema interpreters such as [Arbitrary](/docs/v4/schema/arbitrary) or
[Formatter](/docs/v4/schema/formatter) need a representation for an opaque
declaration. Without one, native Arbitrary derivation fails immediately:

**Example** (Attempting to Generate an Opaque Declaration)
...
Arbitrary.schema(FileSchema)
/*
throws:
Error: Unable to derive an arbitrary for an unsupported Declaration
*/
```

Use `toCodecArbitrary` to link the declaration to a Schema that Arbitrary can
generate. The link decodes generated representation values into the declared
type:

**Example** (Adding an Arbitrary Representation for `File`)
...
const FileSchema = Schema.declare(
  (input: unknown): input is File => input instanceof File,
  {
    identifier: "File",
    toCodecArbitrary: () =>
      Schema.link<File>()(Schema.String, {
        decode: SchemaGetter.transform((name) => new File([], name)),
        encode: SchemaGetter.transform((file) => file.name),
      }),
  },
)

const files = await Effect.runPromise(
  Arbitrary.sampleEffect(Arbitrary.schema(FileSchema), {
    count: 2,
    seed: 1,
  }),
)

files.every((file) => file instanceof File) // => true
```

For the native generation API and its customization options, see the
[Arbitrary](/docs/v4/schema/arbitrary) documentation.
