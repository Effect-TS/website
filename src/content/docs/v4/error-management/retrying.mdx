...
```ts twoslash import.meta.vitest name="retrying-with-a-fixed-delay-1"
...
const result = await Effect.runPromise(repeated)
console.log(result)
/*
Output:
failure
...
```ts twoslash import.meta.vitest name="retrying-a-task-up-to-5-times-1"
...
const result = await Effect.runPromise(Effect.retry(task, { times: 5 }))
/*
Output:
failure
failure
...
```ts twoslash import.meta.vitest name="retrying-until-a-specific-condition-is-met-1"
...
```ts twoslash import.meta.vitest name="retrying-with-fallback-1"
