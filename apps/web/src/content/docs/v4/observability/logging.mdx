...
<Aside type="tip" title="Controlling Log Levels Per Effect">
  Provide `References.MinimumLogLevel` only to the relevant effect to control its logging without
  changing the surrounding program.
</Aside>
...
<Aside type="tip" title="Using ConfigProvider for Testing">
  ...
  values. See [Loading an In-Memory Object](/docs/v4/configuration/#loading-an-in-memory-object) for
  an example.
</Aside>
