...
The article plays the Law & Order sting once on the first click, tap, or key press, subject to the site's saved sound preference. "Off" suppresses automatic playback, "on" allows it, and "system" follows Reduce Motion. Both written "Dun dun." cues provide play controls for deliberate playback regardless of that automatic-playback preference. The local asset, `apps/web/src/assets/blog/astra-vs-the-boys/dun-dun.mp3`, is a 1.3-second clip from [3kh0/soundboard](https://github.com/3kh0/soundboard/blob/main/sounds/DUN_DUN.mp3), where it is listed as "Law and order DUN DUN!". Sebastian suggested [the Myinstants version](https://www.myinstants.com/en/instant/law-and-order-dun-dun/); its MP3 endpoint returned HTTP 403 when downloaded.
