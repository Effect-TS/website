...
A `Layer<RIn, E, ROut>` represents a blueprint for constructing a value of type `ROut`. It takes a value of type `RIn` as input and may potentially produce an error of type `E` during the construction process.
...
Layers compose in the following primary ways: merging and sequencing.
...
Layers can be combined sequentially using the `Layer.provide` combinator:
