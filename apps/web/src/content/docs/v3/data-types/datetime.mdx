...
Similar to [unsafeMake](#unsafemake), but returns an [Option](/docs/v3/api/effect/Option) instead of throwing an error if the input is invalid.
...
Provides the current UTC time as a `Effect<Utc>`, using the [Clock](/docs/v3/requirements-management/default-services/) service.
...
Retrieves the current UTC time immediately using `Date.now()`, without the [Clock](/docs/v3/requirements-management/default-services/) service.
