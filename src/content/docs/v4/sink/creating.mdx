...
To gather all elements from a data stream into a [Chunk](/docs/data-types/chunk/), use the `Sink.collect` sink.
...
To collect a fixed number of elements from a stream into a [Chunk](/docs/data-types/chunk/), use `Sink.take`. This sink stops collecting once it reaches the specified limit.
...
```ts twoslash import.meta.vitest name="summing-elements-in-a-stream-using-fold-left-1"
import { Stream, Sink, Effect } from "effect"
...
const foldWeighted = <A>(cost: (a: A) => number, maxCost: number) =>
  Sink.fold<{ readonly elements: Array<A>; readonly cost: number }, A>(
    ...
    (state, a) => Effect.succeed({ elements: [...state.elements, a], cost: state.cost + cost(a) }),
