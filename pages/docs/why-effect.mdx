...
Notice how looking at the type signature tells you exactly what context the function needs (`never`, as in, no context required), what error(s) it can throw (`Error`), and what success value(s) it returns (`number`). This function no longer throws an exception, and you can cleanly pass on the error to the caller of this function.
