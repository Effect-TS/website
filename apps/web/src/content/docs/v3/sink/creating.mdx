...
The `Sink.take` sink takes the specified number of values from the stream and results in a [Chunk](/docs/v3/api/effect/Chunk) data type.
...
The `Sink.timed` sink executes the stream and measures its execution time, providing the [Duration](/docs/v3/api/effect/Duration).
...
To gather all elements from a data stream into a [Chunk](/docs/v3/api/effect/Chunk), use the `Sink.collectAll` sink.
...
To collect a fixed number of elements from a stream into a [Chunk](/docs/v3/api/effect/Chunk), use `Sink.collectAllN`. This sink stops collecting once it reaches the specified limit.
