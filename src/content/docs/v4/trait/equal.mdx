...
Some examples of such data types include [Option](/docs/data-types/option/), [Result](/docs/data-types/result/), [HashSet](https://effect-ts.github.io/effect/effect/HashSet.ts.html), and [HashMap](https://effect-ts.github.io/effect/effect/HashMap.ts.html).
...
console.log(Equal.equals(alice, new Person(1, "Alice", 30, new Date("2024-06-01"))))
// Output: true
...
The [Data](/docs/data-types/data/) module still provides `Data.Class`, `Data.TaggedClass`, `Data.TaggedError`, and `Data.taggedEnum` for building tagged data types and errors — see the [Data module documentation](/docs/data-types/data/) for details.
...
To perform value-based equality checks, you'll need to use the `Hash*` collection types available in the `effect` package. These collection types, such as [HashSet](https://effect-ts.github.io/effect/effect/HashSet.ts.html) and [HashMap](https://effect-ts.github.io/effect/effect/HashMap.ts.html), use `Equal.equals` for comparisons — which means plain objects, arrays, and other structurally-comparable values are deduplicated automatically, without needing any extra setup.
