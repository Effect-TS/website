...
By default, when a parsing error occurs, the system automatically generates an informative message based on the schema's structure and the nature of the error (see [TreeFormatter](/docs/v3/schema/error-formatters/#treeformatter-default) for more informations).
