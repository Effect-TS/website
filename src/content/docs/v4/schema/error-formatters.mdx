...
```ts twoslash import.meta.vitest name="tree-formatter-default-1"
...
```ts twoslash import.meta.vitest name="customizing-the-output-1"
import { Result, Schema } from "effect"

const Person = Schema.Struct({
  name: Schema.String,
  age: Schema.Number,
}).annotate({ title: "Person" }) // Add a title annotation
...
```ts twoslash import.meta.vitest name="array-formatter-1"
...
if (Result.isFailure(result)) {
  console.error("Decoding failed:")
  console.error(SchemaIssue.makeFormatterStandardSchemaV1()(result.failure.issue).issues)
