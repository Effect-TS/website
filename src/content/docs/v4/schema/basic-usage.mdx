...
Here's how you can [transform](/docs/v4/schema/transformations#transform) the `Shape` schema into another schema that represents a discriminated union:
...
The previous solution works but requires a lot of boilerplate. In v4, you can add the discriminant with `mapFields` and use `Schema.tagDefaultOmit` to provide its decoded default while omitting it during encoding:
...
const schema = Schema.Record(Schema.String.check(Schema.isMinLength(2)), Schema.Finite)
...
A common approach is to use [`Schema.decodeTo`](/docs/v4/schema/transformations#transform) with `SchemaTransformation.transform` to adjust keys during decoding.
...
<Aside type="caution" title="Empty Structs Allow Any Data">
  `Schema.Struct({})` produces the TypeScript type `{}` and accepts any non-nullish value. It
  rejects only `null` and `undefined`.
</Aside>
...
Schema.StructWithRest(
  ...
  [Schema.Record(Schema.String, Schema.Finite), Schema.Record(Schema.String, Schema.Boolean)],
)
/*
throws:
