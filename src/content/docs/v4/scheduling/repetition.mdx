...
<Aside type="note" title="Initial Execution Included">
  ...
  Schedule.duration(Duration.zero))` executes `action` once initially, and if it succeeds, repeats
...
await Effect.runPromise(program).then((n) => console.log(`repetitions: ${n}`))
/*
Output:
success
success
...
await Effect.runPromiseExit(program).then(console.log)
/*
Output:
success
success
