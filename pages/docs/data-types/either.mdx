...
- The `Either.right` function takes a value of type `A` and constructs a `Either<never, A>`:
  ...
  ```ts file=<rootDir>/src/reference/data-types/either/right.ts

  ```

- The `Either.left` function takes a value of type `E` and constructs a `Either<E, never>`:

  ```ts file=<rootDir>/src/reference/data-types/either/left.ts
...
```ts file=<rootDir>/src/reference/data-types/either/guards.ts
...
```ts file=<rootDir>/src/reference/data-types/either/match.ts
...
```ts file=<rootDir>/src/reference/data-types/either/mapRight.ts
...
```ts file=<rootDir>/src/reference/data-types/either/mapLeft.ts
...
```ts file=<rootDir>/src/reference/data-types/either/mapBoth.ts
...
```ts file=<rootDir>/src/reference/data-types/either/interop-effect.ts
