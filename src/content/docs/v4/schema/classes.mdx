...
class WithFilter extends Schema.Class<WithFilter>("WithFilter")(
  ...
  }).check(Schema.makeFilter(({ a, b }) => a >= b || "a must be greater than b")),
) {}
...
```ts twoslash import.meta.vitest name="basic-equality-check-1"
...
```ts twoslash import.meta.vitest name="nested-or-complex-properties-1"
import { Equal, Schema } from "effect"
