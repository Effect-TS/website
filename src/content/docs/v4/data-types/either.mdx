...
/*
Output:
{ _id: 'Result', _tag: 'Failure', failure: 'not a number' }
*/
```
...
/*
Output:
{ _id: 'Result', _tag: 'Success', value: 2 }
*/
...
const failureResult = Result.mapError(Result.fail("not a number"), (s) => s + "!")
console.log(failureResult)
/*
Output:
{ _id: 'Result', _tag: 'Failure', failure: 'not a number!' }
...
Unlike in v3, `Result` is no longer a structural subtype of `Effect`. It implements the `Yieldable` trait instead, so `yield*` still works inside `Effect.gen`, but passing a `Result` directly to an `Effect` combinator (such as `Effect.all`) requires an explicit conversion with `Effect.fromResult`.
...
There is no direct `zipWith` combinator in v4's `Result` module. Combine two `Result` values with a provided function by chaining `Result.flatMap` and `Result.map`: this creates a new `Result` that holds the combined value of both original `Result` values.
...
/*
Output:
{ _id: 'Result', _tag: 'Success', value: { name: 'JOHN', age: 25 } }
*/
```
...
/*
Output:
{ _id: 'Result', _tag: 'Success', value: [ 'John', 25 ] }
*/
...
/*
Output:
{ _id: 'Result', _tag: 'Failure', failure: 'name not found' }
*/
```

## gen

Similar to [Effect.gen](/docs/getting-started/using-generators/), `Result.gen` provides a more readable, generator-based syntax for working with `Result` values, making code that involves `Result` easier to write and understand. This approach is similar to using `async/await` but tailored for `Result`.
