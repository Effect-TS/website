...
export default function TwoslashPatchPortal({  } ) {
        ...
        if(isMounted ) {
            ...
            window.addEventListener("click", (e) => {
                ...
                const { elementStaticXPos, 
                        contentWidth, 
                        allowedContentFullWidth, 
                        elementScrrenXPosRatio} =  changeTwoSlashXPos(currentElement, currentScroll)
    ...
    const clickPropageted : React.MouseEventHandler<HTMLDivElement> = ()  => {
            console.log(1234);
               
    }
    
    console.log('component re-call');
    ...
    return  <>
        ...
        createPortal(
        <>
            <div className="" onClick={clickPropageted}></div>
            <style>
            {
                elementStaticXPos + contentWidth > allowedContentFullWidth
                ?  (elementScrrenXPosRatio < 0.5
                    ? `
                        pre.twoslash data-lsp:hover::before {
                            left: ${screenWidth/8}px !important;
