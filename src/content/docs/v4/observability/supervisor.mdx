...
description: Track the fibers forked by your program using FiberSet and FiberMap, Effect's v4 primitives for managing many fibers together.
sidebar:
  order: 3
---
...
Unlike in v3, there is no standalone `Supervisor` type or `Effect.supervised` function in v4 — the entire `effect/Supervisor` module (`Supervisor.track`, `Effect.supervised`, `effect/FiberStatus`) was removed with no direct replacement. Instead, v4 exposes structured-concurrency primitives that track a set of fibers directly: `FiberSet` for an unkeyed collection, and `FiberMap` for a collection keyed by an arbitrary value. Both automatically remove a fiber once it completes, and interrupt every fiber they still hold when their owning `Scope` closes.
...
const fib = (n: number, set: FiberSet.FiberSet<number>): Effect.Effect<number> =>
  Effect.gen(function* () {
    if (n <= 1) {
      return n
    }
...
<Aside type="tip" title="Reference documentation">
  For the full list of `FiberSet` operations — including `FiberSet.join` (fail the parent if any
  tracked fiber fails) and `FiberSet.awaitEmpty` (wait until every tracked fiber has completed) —
  see the [FiberSet](https://effect-ts.github.io/effect/effect/FiberSet.ts.html) module reference.
</Aside>
...
For the full `FiberMap` API, see the [FiberMap](https://effect-ts.github.io/effect/effect/FiberMap.ts.html) module reference.
