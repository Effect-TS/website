...
```ts twoslash import.meta.vitest name="handling-both-success-and-failure-cases-1"
...
await Effect.runPromise(program1) // => "success: 42"
...
await Effect.runPromise(program2) // => "failure: Uh oh!"
```
...
```ts twoslash import.meta.vitest name="using-effect-ignore-to-discard-values-1"
import { Effect } from "effect"
...
```ts twoslash import.meta.vitest name="handling-success-and-failure-with-side-effects-1"
import { Effect } from "effect"
...
```ts twoslash import.meta.vitest name="handling-different-failure-causes-1"
import { Effect } from "effect"
  ...
  onFailure: (cause) => {
    for (const reason of cause.reasons) {
      switch (reason._tag) {
        ...
        case "Interrupt":
          // Handle interruption
          return `${reason.fiberId} interrupted!`
      }
    }
...
await Effect.runPromise(program) // => "Die: Uh oh!"
```
...
```ts twoslash import.meta.vitest name="handling-different-failure-causes-with-side-effects-1"
    ...
    for (const reason of cause.reasons) {
      switch (reason._tag) {
        case "Fail":
          ...
          return Console.log(`Fail: ${reason.error.message}`)
        case "Die":
          // Handle defects (unexpected errors) by logging the defect
          return Console.log(`Die: ${reason.defect}`)
        case "Interrupt":
          // Handle interruption and log the fiberId that was interrupted
          return Console.log(`${reason.fiberId} interrupted!`)
      }
    }
    // Fallback for other causes
