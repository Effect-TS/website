...
Be cautious when defining index signatures. If they conflict with fixed properties, the resulting TypeScript type may not behave as expected.
...
// Attempting to define a conflicting index signature
// The property "a" expects a number, but the index signature
// allows only strings
const schema = Schema.Struct(
  { a: Schema.Number },
...
The TypeScript compiler will flag this as an error when you define the type manually:
