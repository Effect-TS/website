  ...
  return {
    ...
    load: async ({ config, generateDigest, logger, parseData, store, watcher }) => {
      ...
      for (const entry of entries) {
        const filePath = relative(fileURLToPath(config.root), entry.reflectionPath)
        const data = await parseData({ id: entry.id, filePath, data: entry.data })
        store.set({
          id: entry.id,
          data,
          filePath,
          digest: generateDigest(`${entry.data.revision}:${entry.data.reflectionDigest}`),
        })
      }
