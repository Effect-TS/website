...
In v4, plain JavaScript objects, arrays, tuples, `Map`s, and `Set`s get structural equality with `Equal.equals` by default — no special constructor is required. See [Equal](/docs/trait/equal/) for the full explanation.
...
console.log(Equal.equals(nested, { name: "Alice", nested_field: { value: 42 } }))
// Output: true
```
...
console.log(Equal.equals(nested, { name: "Alice", nested_field: { value: 43 } }))
// Output: false
```
...
<Aside type="tip" title="Deep Comparison">
  `Equal.equals` compares arrays and tuples recursively, including any nested objects or arrays,
...
A plain factory function that returns an object literal gives you a reusable constructor. Since v4 gives plain objects structural equality by default, no special helper is needed for equality or hashing.
...
const BinaryTree = (args: BinaryTree<number>): BinaryTree<number> => ({ ...args })
...
/*
Output:
{ message: 'Cannot find this file', file: 'foo.txt' }
*/
```
