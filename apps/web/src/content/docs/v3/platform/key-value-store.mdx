...
The module exposes a single [service](/docs/v3/requirements-management/services/), `KeyValueStore`, which acts as the gateway for interacting with the store.
...
The module includes two built-in implementations of the `KeyValueStore` interface. Both are provided as [layers](/docs/v3/requirements-management/layers/) that you can inject into your effectful programs.
...
A `SchemaStore` uses a [schema](/docs/v3/schema/introduction/) to validate and convert values. Internally, it serializes data using `JSON.stringify` and deserializes it with `JSON.parse`.
