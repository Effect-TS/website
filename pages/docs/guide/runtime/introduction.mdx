...
A `Runtime<R>` data type represents a Runtime System that can execute effects, in order to execute any effect we need a `Runtime` that contains at least the reqirements of the effect we want to execute.
