...
```ts twoslash import.meta.vitest name="using-config-with-schema-1"
import { Config, ConfigProvider, Effect, Schema } from "effect"

// Define a config that expects a string with at least 4 characters
const myConfig = Config.schema(Schema.String.check(Schema.isMinLength(4)), "Foo")

const result = await Effect.runPromise(
  Effect.provideService(
    myConfig,
    ConfigProvider.ConfigProvider,
    ConfigProvider.fromEnv({ env: { Foo: "test" } }),
  ),
)
result // => "test"
```
...
```ts twoslash
import { Effect, Config, Redacted, Schema } from "effect"
...
const program = Effect.gen(function* () {
  ...
  //      ┌─── Redacted<number>
  //      ▼
  const redacted = yield* Config.schema(Schema.Redacted(Schema.NumberFromString), "SECRET")
...
const program = Effect.gen(function* () {
  const config = yield* Config.schema(Config.Array(Schema.String), "MYARRAY")
  console.log(config)
})
...
```ts twoslash title="index.ts"
import { Config, Effect, HashSet, Schema } from "effect"

const program = Effect.gen(function* () {
  const config = yield* Config.schema(Config.Array(Schema.String), "MYSET").pipe(
    Config.map(HashSet.fromIterable),
  )
  console.log(config)
})
...
```ts twoslash title="index.ts"
import { Config, Effect, HashMap, Schema } from "effect"

const program = Effect.gen(function* () {
  const record = yield* Config.schema(Schema.Record(Schema.String, Schema.String), "MYMAP")
  const config = HashMap.fromIterable(Object.entries(record))
  console.log(config)
})
...
const program = Effect.gen(function* () {
  ...
  const config = yield* Config.schema(
    Schema.String.check(
      Schema.isMinLength(4, { message: "Expected a string at least 4 characters long" }),
    ),
    "NAME",
  )
  console.log(config)
...
```ts twoslash title="orElse.ts" import.meta.vitest name="using-orelse-for-fallback-1"
...
const provider1 = ConfigProvider.fromEnv({ env: { A: "A" } })

// Second provider has B but is missing A
const provider2 = ConfigProvider.fromEnv({ env: { B: "B" } })

// Use `orElse` to fall back from provider1 to provider2
const provider = provider1.pipe(ConfigProvider.orElse(provider2))

Effect.runPromise(Effect.provideService(program, ConfigProvider.ConfigProvider, provider))
```
...
<Aside type="tip">
  In this example, we use `ConfigProvider.fromEnv` to create a configuration provider from a plain
  object. This is particularly useful for testing, as described in the [Mocking Configurations in
  Tests](#mocking-configurations-in-tests) section.
</Aside>
...
```ts twoslash import.meta.vitest name="custom-configuration-types-1"
class HostPort {
  constructor(
...
new HostPort("localhost", 8080).url // => "localhost:8080"
```
...
```ts twoslash import.meta.vitest name="defining-a-custom-configuration-1"
import { Config, ConfigProvider, Effect } from "effect"
...
const result = await Effect.runPromise(
  Effect.provideService(
    config,
    ConfigProvider.ConfigProvider,
    ConfigProvider.fromEnv({ env: { HOST: "localhost", PORT: "8080" } }),
  ),
)
...
```ts twoslash import.meta.vitest name="nested-configurations-1"
class ServiceConfig {
  constructor(
    readonly host: string,
    readonly port: number,
...
new ServiceConfig("localhost", 8080, 30).url // => "localhost:8080"
```
...
```ts twoslash import.meta.vitest name="nested-configurations-2"
import { Config, ConfigProvider, Effect } from "effect"
...
const result = await Effect.runPromise(
  Effect.provideService(
    ...
    ConfigProvider.fromEnv({
      env: { SERVER_HOST: "localhost", SERVER_PORT: "8080", TIMEOUT: "30" },
    }),
  ),
)
result.url // => "localhost:8080"
...
You can achieve this using the `ConfigProvider.fromEnv` constructor.
This method allows you to create a configuration provider from a plain `Record<string, string>`, so you don't have to rely on real process environment variables.
You can then use this mock provider in place of the default one by providing it as the `ConfigProvider.ConfigProvider` service.

**Example** (Mocking a Config Provider for Testing)

```ts twoslash import.meta.vitest name="mocking-a-config-provider-for-testing-1"
import { Config, ConfigProvider, Effect } from "effect"
...
const program = Effect.gen(function* () {
  const hostPort = yield* config
  const message = `Application started: ${hostPort.url}`
  console.log(message)
  message // => "Application started: localhost:8080"
})

// Create a mock config provider using fixed test data
const mockConfigProvider = ConfigProvider.fromEnv({
  env: {
    HOST: "localhost",
    PORT: "8080",
  },
...
Effect.runPromise(Effect.provideService(program, ConfigProvider.ConfigProvider, mockConfigProvider))
```
...
For more complex setups, configurations often include nested keys. `ConfigProvider.fromEnv` uses `_` as the fixed separator for nested keys — this isn't customizable.

**Example** (Providing Nested Configuration Values)

```ts twoslash import.meta.vitest name="providing-nested-configuration-values-1"
import { Config, ConfigProvider, Effect } from "effect"
...
// Mock configuration using '_' as the separator for nested keys
const mockConfigProvider = ConfigProvider.fromEnv({
...
Unlike in v3, the path delimiter for nested configuration keys is fixed to `_` and can't be customized — see [Handling Nested Configuration Values](#handling-nested-configuration-values) above.
...
```ts twoslash import.meta.vitest name="reading-nested-configuration-from-json-1"
import { Config, ConfigProvider, Effect } from "effect"
...
const program = Effect.gen(function* () {
  ...
  const message = `Application started: ${host}:${port}`
  console.log(message)
  message // => "Application started: localhost:8080"
})
...
Effect.runPromise(
  Effect.provideService(
    program,
    ConfigProvider.ConfigProvider,
    ConfigProvider.fromUnknown(JSON.parse(`{"SERVER":{"PORT":8080,"HOST":"localhost"}}`)),
  ),
)
```
