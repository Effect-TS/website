...
```ts file=<rootDir>/src/data-types/duration/creating.ts
...
```ts file=<rootDir>/src/data-types/duration/infinity.ts
...
```ts file=<rootDir>/src/data-types/duration/decode.ts
...
```ts file=<rootDir>/src/data-types/duration/toMillis.ts

```

You can retrieve the value of a duration in nanoseconds using the `toNanos` function:

```ts file=<rootDir>/src/data-types/duration/toNanos.ts
...
```ts file=<rootDir>/src/data-types/duration/unsafeToNanos.ts
...
```ts file=<rootDir>/src/data-types/duration/comparing.ts
...
```ts file=<rootDir>/src/data-types/duration/arithmetic-operations.ts
