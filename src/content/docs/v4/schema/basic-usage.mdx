...
The `Schema.revealCodec` function allows you to view any schema as `Schema<Type, Encoded, Context>`.

**Example** (Expanding a Schema with `asSchema`)

For example, while `Schema.String` is defined as a class with a type of `typeof Schema.String`, using `Schema.revealCodec` provides the schema in its extended form as `Schema<string, string, never>`.
...
//     ▼
const nomalized = Schema.revealCodec(schema)
```
...
const schema = Schema.Tuple([
  ...
  Schema.optionalKey(Schema.Number), // optional element
])
...
const schema = Schema.TupleWithRest(
  Schema.Tuple([Schema.String, Schema.optionalKey(Schema.Number)]), // elements
...
const schema = Schema.TupleWithRest(
  Schema.Tuple([Schema.String, Schema.UndefinedOr(Schema.Number)]), // elements
...
//
//      ┌─── readonly [typeof Schema.String, Schema.UndefinedOr<typeof Schema.Number>]
//      ▼
...
const schema = Schema.Record(Schema.String.check(Schema.isMinLength(2)), Schema.Number)
...
const schema = Schema.Record(Schema.Trim, Schema.NumberFromString)
/*
throws:
...
const schema = Schema.Record(Schema.String, Schema.NumberFromString).pipe(
  Schema.decodeTo(
    // Define the output schema with transformed keys
    Schema.Record(Schema.Trimmed, Schema.Number),
    SchemaTransformation.transform({
...
```ts twoslash import.meta.vitest name="index-signatures-2"
import { Schema } from "effect"
...
Schema.StructWithRest(
  ...
  [Schema.Record(Schema.String, Schema.Number), Schema.Record(Schema.String, Schema.Boolean)],
)
/*
throws:
...
const IndexSignatureProperties = Schema.Record(
  ...
  Schema.String.check(
    Schema.makeFilter((key) => !Object.keys(FixedProperties.fields).includes(key)),
  ),
  Schema.Number,
)
...
const validateSync = Schema.decodeSync(Schema.toType(MyDataSchema))

// Example: Valid instance
console.log(validateSync(new MyData("John")))
// Output: MyData { name: 'John' }

// Example: Invalid instance (empty name)
console.log(validateSync(new MyData("")))
/*
throws:
...
//      |    }>
//      ▼
const PickedSchema = MyStruct.mapFields(Struct.omit(["b"]))
```
...
const schema = Schema.Struct({ a: Schema.String }).mapFields(Struct.map(Schema.optional))
