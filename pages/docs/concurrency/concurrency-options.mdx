...
```ts file=<rootDir>/src/guide/concurrency/concurrency-options/all-default.ts
...
```ts file=<rootDir>/src/guide/concurrency/concurrency-options/all-number.ts
...
```ts file=<rootDir>/src/guide/concurrency/concurrency-options/all-unbounded.ts
...
```ts file=<rootDir>/src/guide/concurrency/concurrency-options/all-inherit-default.ts

```

If you use `Effect.withConcurrency`, it will adopt that specific concurrency configuration.

```ts file=<rootDir>/src/guide/concurrency/concurrency-options/all-inherit-withConcurrency.ts
