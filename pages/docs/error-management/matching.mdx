...
In the `Effect` data type, just like other data types such as [`Exit`](../data-types/exit#matching) and [`Option`](../data-types/option#matching), we have a `Effect.match` function that allows us to handle different cases simultaneously. When working with effects, we also have several functions that enable us to handle both success and failure scenarios.
