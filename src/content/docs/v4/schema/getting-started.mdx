...
<Aside type="tip" title="Returning All Errors">
  By default only the first error is returned. You can use the [`errors`](#receiving-all-errors)
...
<Aside type="note" title="Role of the Encoded Type in Type Guards">
  The encoded type `E`, which is often used in schema transformations, does not affect the creation
  of the type guard. The purpose of the guard is to ensure that the input matches the decoded type
  `T`.
</Aside>
...
<Aside type="note" title="Role of the Encoded Type in Assertions">
  The encoded type `E`, which is often used in schema transformations, does not affect the creation
  of the assertion. Its purpose is to ensure that the input matches the decoded type `T`.
</Aside>
