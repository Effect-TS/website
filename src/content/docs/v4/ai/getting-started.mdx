...
```ts twoslash import.meta.vitest name="define-an-interaction-with-a-language-model-1"
...
class DadJokes extends Context.Service<DadJokes>()("app/DadJokes", {
  make: Effect.gen(function* () {
    // Capturing the model's requirements returns a layer with no
    // remaining requirements
    const gpt = yield* Gpt4o.captureRequirements
    const claude = yield* Claude37.captureRequirements
...
```ts twoslash /{ (OpenAiClient),/ {21-23} collapse={5-17}
...
```ts twoslash /{ (NodeHttpClient) }/ {29} collapse={6-18}
import { OpenAiClient, OpenAiLanguageModel } from "@effect/ai-openai"
import { LanguageModel } from "effect/unstable/ai"
