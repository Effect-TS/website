...
export const RelatedPosts: FC<{slugs: string[]}> = ({slugs}) => {
  ...
  return (
    <div>
      ...
      <h3 className="font-display mb-9 text-2xl sm:text-3xl text-white">Related Posts</h3>
          ...
          (post, index) =>
            post && (
              <Link
                href={post.urlPath}
                key={index}
                className="bg-gradient-to-br from-zinc-800 to-zinc-900 rounded-2xl p-6 transition-transform duration-200 hover:scale-[1.01]"
              >
                <h3 className="font-display text-xl text-white mb-4">{post.title}</h3>
