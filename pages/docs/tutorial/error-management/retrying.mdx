...
const action = Effect.async<never, number, number>((resume) => {
  if (count <= 2) {
    count++
    console.log("failure")
    resume(Effect.fail(count))
  } else {
    console.log("success")
...
const repeated = Effect.retry(action, policy, orElse)
```
