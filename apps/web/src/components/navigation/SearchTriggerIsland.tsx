...
const hasPlatformString = (
  value: unknown,
): value is { readonly platform: string } => {
  return (
    typeof value === "object" &&
    value !== null &&
    "platform" in value &&
...
const detectSearchShortcut = (): SearchShortcut => {
  ...
  const platform = (
    platformFromUserAgentData ?? navigator.platform
  ).toLowerCase()
    ...
    if (openDelayMs > 0) {
      window.setTimeout(() => {
        window.dispatchEvent(
          new CustomEvent(NAVIGATION_EVENTS.SEARCH_OPEN, {
            detail: { source: mode },
          }),
        )
      }, openDelayMs)
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "border-navigation-control-border text-muted-foreground hover:border-navigation-control-border-hover hover:bg-control-background hover:text-foreground",
