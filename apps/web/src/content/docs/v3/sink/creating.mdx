...
The `Sink.take` sink takes the specified number of values from the stream and results in a [Chunk](/docs/v3/data-types/chunk/) data type.
...
The `Sink.timed` sink executes the stream and measures its execution time, providing the [Duration](/docs/v3/data-types/duration/).
...
To gather all elements from a data stream into a [Chunk](/docs/v3/data-types/chunk/), use the `Sink.collectAll` sink.
...
To collect a fixed number of elements from a stream into a [Chunk](/docs/v3/data-types/chunk/), use `Sink.collectAllN`. This sink stops collecting once it reaches the specified limit.
