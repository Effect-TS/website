...
1. Install the package as a development dependency:
   ...
   You'll also need a native TypeScript install alongside it: `typescript` >= 7 (e.g. `typescript@next`) or an alias such as `@typescript/native`.
...
3. Ensure your editor uses the workspace TypeScript version:
   ...
   <Aside type="tip">
     In VS Code or Cursor, you can select the workspace TypeScript version by opening a TypeScript
     file, clicking on the TypeScript version number in the status bar, and selecting "Use Workspace
     Version".
   </Aside>
...
- **Outdated APIs**: Flag usages of APIs that were removed or renamed going from v3 to v4, useful while migrating
...
<Aside type="caution">
  The editor extension does not include the Effect LSP! Installation of that should be performed
  per-project, this allows fine grained control on when to load it, for which projects and with a
  version pinned with your repository lockfile.
</Aside>
...
`DevTools` now ships as part of the core `effect` package (`effect/unstable/devtools`) — no separate install needed. Just import and use it in your Effect app (you'll still need `@effect/platform-node`, or the equivalent package for your runtime, for `NodeRuntime`/`NodeSocket`):
