...
```ts twoslash import.meta.vitest name="parse-1"
...
if (Result.isSuccess(eitherCron)) {
  // Successfully parsed
  console.log("Parsed cron:", eitherCron.success)
} else {
  // Parsing failed
  console.error("Failed to parse cron:", eitherCron.failure.message)
}
```

### unsafeParse

The `parseUnsafe(cronExpression, tz?)` function works like [parse](#parse), but instead of returning an [Either](/docs/data-types/either/), it throws an exception if the input is invalid.

**Example** (Parsing a Cron Expression)

```ts twoslash import.meta.vitest name="unsafe-parse-1"
...
```ts twoslash import.meta.vitest name="match-1"
import { Cron } from "effect"
...
```ts twoslash import.meta.vitest name="next-1"
import { Cron } from "effect"
...
```ts import.meta.vitest name="iterating-over-future-dates-1"
import { Cron } from "effect"
...
```ts twoslash collapse={4-30} import.meta.vitest name="cron-1"
      ...
      Effect.repeat(
        schedule.pipe(
          ...
          Schedule.tap(({ now, output }) =>
            Console.log(i === 11 ? "..." : new Date(now + Duration.toMillis(output))),
          ),
        ),
      ),
      Effect.forkChild,
