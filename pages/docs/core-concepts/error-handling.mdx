...
#### Modifying the error channel

Sometimes you don't want to actually handle the error, but simplify modify it. For example, you might want to add some additional information to the error, or you might want to change the error type altogether.
...
If you are using Effect with a Promise-based API, you can use the `Effect.promise` constructor to convert a Promise into an Effect. If the Promise rejects, the resulting Effect will throw with the error that the Promise rejected with. You can also use other variants such as `Effect.tryPromise` or `Effect.tryCatchPromise` to convert Promise rejections into typed errors in the Effect error channel.
