---
pagefind: false
title: This Week in Effect - 2026-01-30
excerpt: Actor Model | Effect.ts Cluster. Effect Office Hours and Refactoring Twitch-Spotify Integration with Effect Cluster coding session.
date: 2026-01-30
authors:
  - davide_scognamiglio
  - mirela_prifti
...
### Effect SQL
- [Fix(@effect/sql-pg): readded stream to PgClientConfig](https://github.com/Effect-TS/effect/pull/5998) (Bug Fix)

&nbsp;

## Community Events

Don't miss our upcoming [Effect Berlin Meetup #2 🇩🇪](https://luma.com/effect-community?e=calev-yq3Meujax7IFE8j) on January 29, and the [Effect Paris Meetup #6 🇫🇷](https://www.meetup.com/fr-fr/effect-paris/events/312953757/) on February 3.
...
- **Actor Model | Effect.ts Cluster** by Lucas Barake.

<YouTube
  id="https://www.youtube.com/embed/Pp9ufBrtNgE?si=wTSdgueQRKHE_bAW"
  title="Actor Model | Effect.ts Cluster"
/>

&nbsp;

- **Foldkit** - a frontend framework built on Effect that follows The Elm Architecture - now has a proper [docs website](https://foldkit.dev/).

&nbsp;

- [`effect-inngest`](https://github.com/erikshestopal/effect-inngest) - A TypeScript SDK combining Effect with Inngest's durable workflow execution platform. Features full type inference, Effect-native steps, dependency injection via Effect's Layer system, and Schema validation — with zero boilerplate and multi-runtime support (Bun, Node.js, Cloudflare Workers). Project by [erikshestopal](https://github.com/erikshestopal).

&nbsp;

- [Effect Best Practices](https://skills.sh/makisuo/skills/effect-best-practices) - A code style guide and linting skill for AI coding assistants that enforces consistent patterns when working with Effect codebases, covering service architecture, error handling, type safety, frontend state management, configuration, and observability. Project by [makisuo](https://github.com/makisuo).

&nbsp;

- [serial.dev](https://serial.dev/#try) - a multiplayer coding platform with fast coding sandboxes, built with Effect on the backend, is now in [public preview](https://x.com/NatalieMarleny/status/2015103211946823733?s=20).

&nbsp;

- [Giuseppe Antonio Corona](https://github.com/peppineddu5) created a [fork](https://github.com/peppineddu5/effect-worker-mono) of the `effect-worker-mono` repo by **Backpine Labs** and made some changes:
   - Migrated to Cloudflare D1 for serverless SQL storage
   - Built the frontend with SvelteKit on the front-end
   - MsgPack-based RPC
   - Included a TanStack Query wrapper for Effect + Svelte
   - Made it easy to call both effect-api and effect-rpc from the client using the proxy

&nbsp;

- [`effect-machine`](https://github.com/cevr/effect-machine) - A library for building type-safe state machines using Effect. Features a schema-first approach with compile-time transition validation, state-scoped effects with auto-cancellation, parameterized guards, deterministic testing, and persistence support for snapshots and event sourcing. Project by [cevr](https://github.com/cevr).

&nbsp;

- [`voltaire-effect`](https://voltaire-effect.tevm.sh/) - A TypeScript library combining Effect with Voltaire Ethereum primitives for type-safe smart contract interactions. Features composable error handling, built-in retry policies, timeout handling, and WASM-optimized cryptography. Part of the [TEVM](https://tevm.sh/) ecosystem.

&nbsp;

# Effect Content Update

- **Office Hours 13: andThen vs flatMap, HTTP Client vs HTTP API, AI workflow and Ralph, etc.** and **Refactoring Twitch-Spotify Integration with Effect Cluster** are now on our YouTube Channel.

<YouTube
  id="https://www.youtube.com/embed/uJvN-2OCU-E?si=eXsGDZ-OzetNChYC"
  title="Office Hours 13: andThen vs flatMap, HTTP Client vs HTTP API, AI workflow and Ralph, etc."
/>

&nbsp;

<YouTube
  id="https://www.youtube.com/embed/cLxqMczCDeU?si=gGPgTaj5cyuUSe2Y"
  title="Refactoring Twitch-Spotify Integration with Effect Cluster"
/>

&nbsp;

- [Effect Office Hours 14](https://x.com/EffectTS_/status/2016527048689487984) went live on Wednesday on X(Twitter)! The edited version will be on our YouTube channel soon.
