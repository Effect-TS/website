...
The `parse(cronExpression, tz?)` function safely parses a cron string into a `Cron` instance. It returns a [Result](/docs/data-types/result/), which will contain either the parsed `Cron` or a parsing error.
...
Result.isSuccess(eitherCron) // => true
```

### unsafeParse

The `parseUnsafe(cronExpression, tz?)` function works like [parse](#parse), but instead of returning a [Result](/docs/data-types/result/), it throws an exception if the input is invalid.
...
const firstTen = Array.from({ length: 10 }, () => iterator.next().value?.toISOString())
