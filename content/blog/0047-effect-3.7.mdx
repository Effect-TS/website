...
Effect.gen(function*() {
  ...
  // run a fiber and propogate interrupts
...
The following apis now preserve non-empty arrays:
...
This could cause finalizers to leak, so now the behavior must be explicitly requested
