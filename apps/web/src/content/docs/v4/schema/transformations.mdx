...
const ReadonlySetFromArray = <S extends Schema.Constraint>(itemSchema: S) =>
  Schema.Array(itemSchema).pipe(
    Schema.decodeTo(
      ...
      SchemaTransformation.transform({
        decode: (items: ReadonlyArray<S["Type"]>) => new Set(items),
        encode: (set: ReadonlySet<S["Type"]>): ReadonlyArray<S["Type"]> => Array.from(set.values()),
      }),
    ),
  )
...
<Aside type="note" title="Why Schema.toType is used">
  ...
  [Schema.toType](/docs/v4/schema/projections#totype). This is because the decoding/encoding of the
...
export const ColorFromString = Schema.String.pipe(
  Schema.decodeTo(Color, {
    decode: SchemaGetter.transformOrFail((input) => {
      ...
      return Effect.fail(new SchemaIssue.InvalidValue({ message: "Unsupported color" }))
    }),
...
export const ContentDigest = Content.pipe(
  Schema.decodeTo(Sha256Digest, {
    decode: SchemaGetter.transform((content) => createHash("sha256").update(content).digest("hex")),
    encode: SchemaGetter.forbidden(() => "A SHA-256 digest cannot be encoded as its source."),
  }),
)
...
const split = (separator: string) =>
  Schema.String.pipe(
    Schema.decodeTo(
      ...
      SchemaTransformation.transform({
        decode: (value) => value.split(separator),
...
const ComposedSchema = split(",").pipe(Schema.decodeTo(FiniteArrayFromStringArray))
...
<Aside type="tip" title="Lowercase And Lowercased">
  To validate without transforming, use `Schema.String.check(Schema.isLowercased())`.
</Aside>
...
<Aside type="tip" title="Uppercase And Uppercased">
  To validate without transforming, use `Schema.String.check(Schema.isUppercased())`.
</Aside>
...
<Aside type="tip" title="Capitalize And Capitalized">
  To validate without transforming, use `Schema.String.check(Schema.isCapitalized())`.
</Aside>
...
<Aside type="tip" title="Uncapitalize And Uncapitalized">
  To validate without transforming, use `Schema.String.check(Schema.isUncapitalized())`.
</Aside>

### JSON Strings

`Schema.UnknownFromJsonString` decodes JSON text with `JSON.parse` and encodes values with `JSON.stringify`.
...
Use `Schema.fromJsonString` to decode the parsed value through another schema.
