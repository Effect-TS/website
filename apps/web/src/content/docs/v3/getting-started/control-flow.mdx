...
Here we are using the [Option](/docs/v3/api/effect/Option) data type to represent the absence of a valid value.
...
In this example, the [Option](/docs/v3/api/effect/Option) data type is used to represent the presence or absence of a valid value. If the condition evaluates to `true` (in this case, if the weight is non-negative), the effect is executed and wrapped in a `Some`. Otherwise, the result is `None`.
...
If any effect fails, it stops execution (short-circuiting) and propagates the error. To change this behavior, you can use the [`mode`](#the-mode-option) option, which allows all effects to run and collect results as [Either](/docs/v3/api/effect/Either) or [Option](/docs/v3/api/effect/Option).
