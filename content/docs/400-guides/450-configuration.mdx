...
```ts filename="primitives.ts" file=<rootDir>/src/configuration/primitives-gen.ts
...
```ts filename="primitives.ts" file=<rootDir>/src/configuration/primitives-pipe.ts
...
```ts filename="withDefault.ts" /Config.withDefault/ file=<rootDir>/src/configuration/withDefault-gen.ts
...
```ts filename="withDefault.ts" /Config.withDefault/ file=<rootDir>/src/configuration/withDefault-pipe.ts
...
```ts filename="HostPort.ts" file=<rootDir>/src/configuration/HostPort.ts#L3-L9

```

We can define a configuration for this data type by combining primitive configs for `string` and `number`:

```ts filename="HostPort.ts" file=<rootDir>/src/configuration/HostPort.ts
...
```ts filename="ServiceConfig.ts" file=<rootDir>/src/configuration/ServiceConfig.ts
...
```ts /Config.nested/ filename="ServiceConfig.ts" file=<rootDir>/src/configuration/ServiceConfig-nested.ts#L8-
const config: Config.Config<ServiceConfig> = Config.all(Config.nested(HostPort.config, "HOSTPORT"), Config.float("TIMEOUT")).pipe(
...
```ts filename="mockConfigProvider.ts" file=<rootDir>/src/configuration/mockConfigProvider.ts
...
```ts file=<rootDir>/src/configuration/secret.ts
