...
A schema is an immutable value that describes the structure of your data. In v4, the same schema value can be viewed through different interfaces, depending on which type-level information an API needs:
...
<Aside type="note" title="Type Parameter Abbreviations">
  In the Effect ecosystem, you may encounter the type parameters of `Codec` abbreviated as `T`, `E`,
  `RD`, and `RE`: decoded **T**ype, **E**ncoded type, decoding services, and encoding services.
</Aside>
