...
The `Equal` module provides a simple and convenient way to define and check for equality between two values in TypeScript. This tutorial will guide you through the basics of using `Equal` and show you some code examples.
...
```ts file=<rootDir>/src/trait/equal/equals.ts
...
```ts filename="Person.ts" file=<rootDir>/src/trait/equal/Person.ts
...
```ts file=<rootDir>/src/trait/equal/custom.ts
...
```ts file=<rootDir>/src/trait/equal/data.ts
...
```ts file=<rootDir>/src/trait/equal/data-arrays.ts
