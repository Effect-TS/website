...
import type { ApiReferenceEntry } from "./schema.ts"
import { ApiReferenceDatasetManifest, ApiReferencePackageManifest } from "./schema.ts"
...
export async function loadApiReferenceDataset(
  ...
  for (const version of versions) {
    ...
    for (const packageEntry of dataset.packages) {
      ...
      const packageManifest = ApiReferencePackageManifest.parse(await readJson(packageManifestPath))
      if (
