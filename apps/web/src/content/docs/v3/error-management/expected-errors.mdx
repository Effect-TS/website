...
The `Effect.either` function transforms an `Effect<A, E, R>` into an effect that encapsulates both potential failure and success within an [Either](/docs/v3/api/effect/Either) data type:
...
Transforms an effect to encapsulate both failure and success using the [Option](/docs/v3/api/effect/Option) data type.
