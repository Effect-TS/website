...
The `effect/unstable/persistence/KeyValueStore` module provides a robust and effectful interface for managing key-value pairs.
...
KeyValueStore.KeyValueStore.key // => "effect/persistence/KeyValueStore"
```
...
| -------------------- | ------------------------------------------------------------------------ |
| **get**              | Returns the value as `string`, or `undefined` if the key does not exist. |
...
```ts twoslash import.meta.vitest name="basic-operations-with-a-key-value-store-1"
...
By default, `KeyValueStore` works with `string` and `Uint8Array` values. To store other types such as objects, numbers, or booleans, use the `toSchemaStore` function to create a `SchemaStore`.
...
```ts twoslash import.meta.vitest name="storing-a-typed-object-using-a-schema-1"
import {
  KeyValueStore,
  layerMemory,
  toSchemaStore,
...
const program = Effect.gen(function* () {
  ...
  const kv = toSchemaStore(yield* KeyValueStore, Person)
  ...
  yield* kv.get("user1") // => Option.some({ name: "Alice", age: 30 })
})
