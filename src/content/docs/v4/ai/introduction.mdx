...
Effect’s AI integrations are built around the idea of **provider-agnostic programming**. Instead of hardcoding calls to a specific LLM provider's API, you describe your interaction using the services provided by `effect/unstable/ai`.
...
Defines the core abstractions for interacting with LLM provider services. This module namespace, part of the `effect` package itself, defines the generic services and helper utilities needed to build AI-powered applications in a provider-agnostic way.
...
For detailed API documentation, see the [API Reference](/docs/v4/api/effect/unstable/ai/LanguageModel).

### `@effect/ai-openai`

Concrete implementations of services from `effect/unstable/ai` backed by the [OpenAI API](https://platform.openai.com).
...
For detailed API documentation, see the [API Reference](/docs/v4/api/ai-openai).

### `@effect/ai-anthropic`

Concrete implementations of services from `effect/unstable/ai` backed by the [Anthropic API](https://docs.anthropic.com/en/api/getting-started).

Supported services include:

- `LanguageModel` (via Anthropic's [Messages API](https://docs.anthropic.com/en/api/messages))

For detailed API documentation, see the [API Reference](/docs/v4/api/ai-anthropic).
