...
`Effect.catch` allows you to attempt to run an effect, and if it fails, you
...
**Example** (Handling Fallback with `Effect.catch`)

```ts twoslash import.meta.vitest name="handling-fallback-with-effect-catch-1"
...
const program1 = Effect.catch(success, () => fallback)
console.log(Effect.runSync(program1))
Effect.runSync(program1) // => "success"

// Try the failure effect first, fallback is used
const program2 = Effect.catch(failure, () => fallback)
console.log(Effect.runSync(program2))
Effect.runSync(program2) // => "fallback"
```

## orElseFail

`Effect.mapError` allows you to replace the failure from one effect with a
...
**Example** (Replacing Failure with `Effect.mapError`)

```ts twoslash import.meta.vitest name="replacing-failure-with-effect-maperror-1"
...
const program = Effect.mapError(validate(-1), () => "invalid age")
...
*/
Effect.runSyncExit(program) // => Exit.fail("invalid age")
```
...
```ts twoslash import.meta.vitest name="replacing-failure-with-success-using-effect-orelsesucceed-1"
import { Effect, Exit } from "effect"
...
```ts twoslash import.meta.vitest name="finding-configuration-with-fallbacks-1"
...
*/
result // => { host: "node3.example.com", port: 8080, apiKey: "12345-abcde" }
```
