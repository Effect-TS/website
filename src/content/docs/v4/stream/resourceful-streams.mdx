...
const stream = Stream.scoped(
  Stream.fromEffect(Effect.acquireRelease(open("file.txt"), (file) => file.close)),
).pipe(Stream.flatMap((file) => Stream.fromEffect(file.getLines)))
...
await Effect.runPromise(Stream.runCollect(program)) // => [undefined, undefined]
```
...
```ts twoslash import.meta.vitest name="ensuring-1"
import { Stream, Console, Effect } from "effect"

const program = Stream.fromEffect(Console.log("Application Logic.")).pipe(
  Stream.concat(
    Stream.scoped(
      Stream.fromEffect(Effect.addFinalizer(() => Console.log("Finalizing the stream"))),
    ),
  ),
