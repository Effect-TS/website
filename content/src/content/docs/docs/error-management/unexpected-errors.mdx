...
It's important to understand that `catchAllDefect` can only handle defects, not [expected errors](/docs/error-management/expected-errors/) (such as those caused by `Effect.fail`) or [interruptions](/docs/concurrency/basic-concurrency/#interruptions) in execution (such as when using `Effect.interrupt`).
