...
document.schema.anyOf?.length // => 2
document.schema.anyOf?.[0].required // => ["_tag", "value"]
document.schema.anyOf?.[1].required // => ["_tag"]
```
...
document.schema.required // => undefined
document.schema.properties?.name // => { type: "string" }
document.schema.properties?.nickname // => { anyOf: [{ type: "string" }, { type: "null" }] }
```
...
document.schema.properties?.name // => { $ref: "#/$defs/Name" }
document.definitions // => { Name: { type: "string" } }
```
...
const Category = Schema.Struct({
  name: Schema.String,
  categories: Schema.Array(Schema.suspend((): Schema.Codec<Category> => Category)),
}).annotate({ identifier: "Category" })

const document = Schema.toJsonSchemaDocument(Category)

document.schema // => { $ref: "#/$defs/Category" }
document.definitions.Category.type // => "object"
document.definitions.Category.required // => ["name", "categories"]
document.definitions.Category.properties?.categories // => { type: "array", items: { $ref: "#/$defs/Category" } }
```
