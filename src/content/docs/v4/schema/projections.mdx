...
declare const toType: <S extends Schema.Constraint>(schema: S) => Schema.toType<S>
```
...
```ts showLineNumbers=false
declare const toEncoded: <S extends Schema.Constraint>(schema: S) => Schema.toEncoded<S>
```
