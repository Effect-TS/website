...
const getNames: Effect.Effect<never, never, List.List<string>> = (() => {
  const loop = (
    ...
    pipe(
      ...
      Effect.flatMap((name) => {
        if (name === "q") {
          ...
          return loop(List.prepend(state, name))
        }
      })
    )
