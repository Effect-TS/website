...
```ts twoslash import.meta.vitest name="creating-an-effect-that-can-fail-1"
...
```ts twoslash import.meta.vitest name="automatically-tracking-errors-1"
import { Effect, Random, Data } from "effect"
...
```ts twoslash {14-15} import.meta.vitest name="short-circuiting-behavior-1"
...
await Effect.runPromiseExit(program) // => Exit.fail("Something went wrong!")
```
...
By yielding a `Result`, we gain the ability to "pattern match" on this type to handle both failure and success cases within the generator function.

**Example** (Using `Effect.result` to Handle Errors)

```ts twoslash import.meta.vitest name="using-effect-result-to-handle-errors-1"
import { Effect, Result, Random, Data } from "effect"
...
const recovered = Effect.gen(function* () {
  //      ┌─── Result<string, HttpError | ValidationError>
  //      ▼
  const failureOrSuccess = yield* Effect.result(program)
  if (Result.isFailure(failureOrSuccess)) {
    // Failure case: you can extract the error from the `failure` property
    const error = failureOrSuccess.failure
    return `Recovering from ${error._tag}`
  } else {
    // Success case: you can extract the value from the `success` property
...
We can make the code less verbose by using the `Result.match` function, which directly accepts the two callback functions for handling errors and successful values:

**Example** (Simplifying with `Result.match`)

```ts twoslash collapse={3-19} import.meta.vitest name="simplifying-with-result-match-1"
import { Effect, Result, Random, Data } from "effect"
...
const recovered = Effect.gen(function* () {
  ...
  //      ▼
  const failureOrSuccess = yield* Effect.result(program)
  return Result.match(failureOrSuccess, {
    onFailure: (error) => `Recovering from ${error._tag}`,
    onSuccess: (value) => value, // Do nothing in case of success
  })
})
...
```ts twoslash import.meta.vitest name="using-effect-option-to-handle-errors-1"
...
await Effect.runPromiseExit(maybe1) // => Exit.succeed(Option.some(1))

const maybe2 = Effect.option(Effect.fail("Uh oh!"))

Effect.runPromiseExit(maybe2).then(console.log)

await Effect.runPromiseExit(maybe2) // => Exit.succeed(Option.none())
...
await Effect.runPromiseExit(maybe3) // => Exit.die("Boom!")
```
...
The `Effect.catch` function catches any errors that may occur during the
...
<Aside type="note" title="Recoverable Errors Only">
  `Effect.catch` only handles recoverable errors. It will not recover from unrecoverable defects.
...
```ts twoslash import.meta.vitest name="providing-recovery-logic-for-recoverable-errors-1"
import { Effect, Random, Data } from "effect"
...
const recovered = program.pipe(
  Effect.catch((error) => Effect.succeed(`Recovering from ${error._tag}`)),
)
...
The `Effect.catchCause` function allows you to handle all errors, including
...
```ts twoslash import.meta.vitest name="recovering-from-all-errors-1"
...
await Effect.runPromise(recovered) // => "Recovered from a regular error"
```
...
The [`Effect.result`](#either) function, which was previously shown as a way to catch all errors, can also be used to catch specific errors.

By yielding a `Result`, we gain the ability to "pattern match" on this type to handle both failure and success cases within the generator function.

**Example** (Handling Specific Errors with `Effect.result`)

```ts twoslash import.meta.vitest name="handling-specific-errors-with-effect-result-1"
import { Effect, Random, Result, Data } from "effect"
...
```ts twoslash collapse={3-17} {28-30} import.meta.vitest name="either-2"
import { Effect, Random, Result, Data } from "effect"
...
`Effect.catchFilter` lets you selectively catch and handle errors of certain
...
**Example** (Handling Specific Errors with `Effect.catchFilter`)

```ts twoslash import.meta.vitest name="handling-specific-errors-with-effect-catchfilter-1"
import { Effect, Random, Filter, Data } from "effect"
...
const recovered = program.pipe(
  // Only handle HttpError errors
  Effect.catchFilter(Filter.tagged("HttpError"), (error) =>
...
In the code above, `Effect.catchFilter` takes a function that examines the error and decides whether to attempt recovery or not. If the error matches a specific condition, recovery can be attempted by returning `Option.some(effect)`. If no recovery is possible, you can simply return `Option.none()`.

It's important to note that while `Effect.catchFilter` lets you catch specific errors, it doesn't alter the error type itself.
...
`Effect.catchIf` works similarly to [`Effect.catchFilter`](#catchsome), but it allows you to
...
```ts twoslash import.meta.vitest name="catching-specific-errors-with-a-predicate-1"
...
```ts twoslash collapse={3-19} {25-26} import.meta.vitest name="workaround-for-typescript-versions-lt-5-5-1"
import { Data, Effect, Random } from "effect"
...
```ts twoslash import.meta.vitest name="handling-errors-by-tag-1"
import { Effect, Random, Data } from "effect"
...
```ts twoslash collapse={3-19} {28-30} import.meta.vitest name="handling-multiple-error-types-with-catchtag-1"
import { Effect, Random, Data } from "effect"
...
```ts twoslash import.meta.vitest name="handling-multiple-tagged-error-types-at-once-1"
import { Effect, Random, Data } from "effect"
...
Effect.runFork(myfunc(100).pipe(Effect.catchCause(Effect.logError)))
/*
Output:
got: 100
