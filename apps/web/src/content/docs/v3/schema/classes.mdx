...
When working with schemas, you have a choice beyond the [Schema.Struct](/docs/v3/schema/basic-usage/#structs) constructor.
...
- **Value Hashing and Equality**: Utilize the built-in capability for checking value equality and applying hashing (thanks to `Class` implementing [Data.Class](/docs/v3/api/effect/Data#Class)).

<Aside type="caution" title="Class Schemas Are Transformations">
  Classes defined with `Schema.Class` act as
  [transformations](/docs/v3/schema/transformations/). See [Class Schemas are
...
This behavior is similar to how we handle other class-based APIs like [Context.Tag](/docs/v3/requirements-management/services/#creating-a-service).
...
Class schemas [transform](/docs/v3/schema/transformations/) a struct schema into a [declaration](/docs/v3/schema/advanced-usage/#declaring-new-data-types) schema that represents a class type.
...
Instances of classes created with `Schema.Class` support the [Equal](/docs/v3/trait/equal/) trait through their integration with [Data.Class](/docs/v3/api/effect/Data#Class). This enables straightforward value comparisons, even across different instances.
...
Defining a class with `Schema.Class` is similar to creating a [transformation](/docs/v3/schema/transformations/) schema that converts a struct schema into a [declaration](/docs/v3/schema/advanced-usage/#declaring-new-data-types) schema representing the class type.
...
You can also create classes that extend [TaggedClass](/docs/v3/api/effect/Data#TaggedClass) and [TaggedError](/docs/v3/api/effect/Data#TaggedError) from the `effect/Data` module.
