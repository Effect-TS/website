...
Because `throwAnError` has a return type of `never`, TypeScript knows that the
`name` variable can't be `null` at the point where we call `toUpperCase`.
Without the `never`, TypeScript would give us an error on the `toUpperCase`
call.
