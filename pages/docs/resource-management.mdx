...
## Pattern: Chained Operations with Reversal for Failure
...
We then create simple service implementations to test the behavior of our Workspace code. Note that each service implementation accepts a `fail` boolean, which allows us to test different error scenarios.

```ts filename="WorkspaceTest.ts" file=<rootDir>/src/guide/resource-management/pattern/WorkspaceTest.ts

```

Let's examine the test results for the scenario where all `fail` arguments are set to `false` (happy path):

```bash filename="Terminal"
... message="[S3] creating bucket"
... message="[ElasticSearch] creating index"
... message="[Database] creating entry for bucket <bucket.name> and index <index.id>"
{ _tag: 'Right', right: undefined }
```

In this case, all operations succeed, and we see a successful result with `right(undefined)`.
...
const ctx = Context.empty().pipe(
  ...
  Context.add(Services.Database, DatabaseTest(true))
)
```
...
... message="[ElasticSearch] creating index"
... message="[Database] creating entry for bucket <bucket.name> and index <index.id>"
... message="[ElasticSearch] delete index <index.id>"
... message="[S3] delete bucket <bucket.name>"
...
const ctx = Context.empty().pipe(
  Context.add(Services.S3, S3Test(false)),
  Context.add(Services.ElasticSearch, ElasticSearchTest(true)),
