...
```ts filename="error-tracking.ts" file=<rootDir>/src/guide/error-management/expected-errors/error-tracking-gen.ts#L1-L27
...
```ts filename="error-tracking.ts" file=<rootDir>/src/guide/error-management/expected-errors/error-tracking.ts#L1-L23
...
The [`Effect.either`](error-channel-operations#exposing-errors-in-the-success-channel) function converts an `Effect<R, E, A>` into another effect where both its failure (`E`) and success (`A`) channels have been lifted into an `Either<E, A>` data type:
...
If **all** the errors are **tagged** we can catch a specific error using the `Effect.catchTag` function:
