...
`A` represents the success value of the computation.

`E` represents the type of error, encapsulated within a `MicroCause`.
...
/*
Example Output:
{ _id: 'Either', _tag: 'Right', right: 'Recovering from HttpError' }
*/
```
...
Example Output:
{
  _id: 'Either',
  _tag: 'Right',
  right: 'Recovering from HttpError'
}
*/
```
...
/*
Output:
{ _id: 'Either', _tag: 'Right', right: 18 }
*/
```
...
When you fork an effect, it starts executing the effect on a new fiber, giving you a reference (`Handle`) to this newly-created fiber.
...
//      ┌─── Micro<Handle<number, never>, never, never>
//      ▼
...
Micro.runPromise(program)
// Output: { _id: 'Either', _tag: 'Right', right: 55 }
```
...
const program = Micro.gen(function* () {
  ...
  // Interrupt the fiber and get the MicroExit result
