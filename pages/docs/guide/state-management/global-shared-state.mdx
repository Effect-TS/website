...
const program = Effect.flatMap(Counter.make, (counter) =>
  pipe(
    ...
    Effect.flatMap((value) =>
      Effect.logInfo(`This counter has a value of ${value}.`)
    )
  )
)
...
const program = Effect.gen(function* (_) {
  ...
  yield* _(Effect.logInfo(`This counter has a value of ${value}.`))
})
...
const program = Effect.flatMap(Counter.make, (counter) => {
  const logCounter = <R, E, A>(action: Effect.Effect<R, E, A>) =>
    pipe(
      counter.get,
      Effect.flatMap((value) => Effect.logInfo(`get: ${value}`)),
      Effect.flatMap(() => action)
    )

  return pipe(
    counter.inc,
    Effect.zipPar(logCounter(counter.inc)),
    Effect.zipPar(logCounter(counter.dec)),
    Effect.zipPar(logCounter(counter.inc)),
    Effect.flatMap(() => counter.get),
    Effect.flatMap((value) =>
      Effect.logInfo(`This counter has a value of ${value}.`)
...
const program = Effect.gen(function* (_) {
  ...
  const logCounter = <R, E, A>(effect: Effect.Effect<R, E, A>) =>
    Effect.gen(function* (_) {
      const value = yield* _(counter.get)
      yield* _(Effect.logInfo(`get: ${value}`))
