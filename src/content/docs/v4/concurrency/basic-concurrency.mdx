...
If you want to handle the result of whichever task completes first, whether it succeeds or fails, you can use the `Effect.result` function. This function wraps the result in a [Result](/docs/data-types/result/) type, allowing you to see if the result was a success (`Success`) or a failure (`Failure`):
