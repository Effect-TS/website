...
In the same way it is possible to leverage combinators such as [Effect.fail](/docs/v3/getting-started/creating-effects/#fail) to create values of type `Effect<never, E, never>` the Effect library provides tools to create defects.
...
The `Effect.exit` function transforms an `Effect<A, E, R>` into an effect that encapsulates both potential failure and success within an [Exit](/docs/v3/data-types/exit/) data type:
...
However, it does not handle expected errors (like those from [Effect.fail](/docs/v3/getting-started/creating-effects/#fail)) or
execution interruptions (like those from [Effect.interrupt](/docs/v3/concurrency/basic-concurrency/#interrupt)).
